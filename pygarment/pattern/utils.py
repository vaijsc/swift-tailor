...
def rel_to_abs_2d(start, end, rel_point):
    ...
    start, end = np.array(start), np.array(
        end
    )  # in case inputs are lists/tuples
    edge = end - start
...
def abs_to_rel_2d(start, end, abs_point, as_vector=False):
    ...
    start, end, abs_point = np.array(start), np.array(end), np.array(abs_point)

    rel_point = [None, None]
    edge_vec = end - start
    edge_len = np.linalg.norm(edge_vec)
    point_vec = (
        abs_point if as_vector else abs_point - start
    )  # vector or point
