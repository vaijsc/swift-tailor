...
def float_representer(dumper, data):
    ...
    if data != data or (data == 0.0 and data == 1.0):
        ...
        value = f"{data:.3g}"
        if (
            "." not in value or "e" in value
        ):  # e only appears for large int numbers with this precision
...
class Properties:
        ...
        try:
            ...
            if extention == ".json":
                with open(filename, "w") as f_json:
                    json.dump(
                        self.properties, f_json, indent=2, sort_keys=True
                    )
            elif extention == ".yaml":
                ...
                raise ValueError(
                    f"{self.__class__.__name__}::ERROR::Unsupported file type on serialization: {extention}"
                )
            ...
            raise RuntimeError(
                "Error occured while saving properties. Backup version is saved instead"
            )

    def merge(
        self, filename="", clean_stats=False, re_write=True, adding_tag="added"
    ):
        ...
        * re_write=False will keep both properties if their values are
        ...
        self._recursive_dict_update(
            self.properties, new_props, re_write, adding_tag
        )
    ...
    def is_fail_section(self, dataname):
        ...
        for section_key in self.properties:
            ...
            if (
                ...
                and "stats" in section
                and ("fails" in section["stats"])
            ):
                if isinstance(section["stats"]["fails"], dict):
                    for key in section["stats"]["fails"]:
                        if not isinstance(
                            section["stats"]["fails"][key], list
                        ):
                        ...
                        # expects a list as value
                        if dataname in section["stats"]["fails"][key]:
                            return True, key
    ...
    def count_fails(self, log=False):
        ...
        for section_key in self.properties:
            ...
            if (
                ...
                if log:
                    section["stats"]["fails_count"] = len(
                        list(set(section_fails))
                    )
        ...
        for section in self.properties.values():
            ...
            if isinstance(section, dict) and "stats" in section:
                if key in section["stats"]:
                    ...
                    if (
                        isinstance(stats_values, list)
                        and len(stats_values) > 0
                        and isinstance(stats_values[0], Number)
                    ):
                        if log_sum:
                            section["stats"][key + "_sum"] = (
                                str(timedelta(seconds=sum(stats_values)))
                                if as_time
                                else sum(stats_values)
                            )
                            updated = True
                        if log_avg:
                            section["stats"][key + "_avg"] = sum(
                                stats_values
                            ) / len(stats_values)
                            if as_time:
                                section["stats"][key + "_avg"] = str(
                                    timedelta(
                                        seconds=section["stats"][key + "_avg"]
                                    )
                                )
                            updated = True
                        if log_median:
                            section["stats"][key + "_med"] = (
                                str(
                                    timedelta(
                                        seconds=np.percentile(stats_values, 50)
                                    )
                                )
                                if as_time
                                else float(np.percentile(stats_values, 50))
                            )
                            updated = True
                        if log_80:
                            section["stats"][key + "_p80"] = (
                                str(
                                    timedelta(
                                        seconds=np.percentile(stats_values, 80)
                                    )
                                )
                                if as_time
                                else float(np.percentile(stats_values, 80))
                            )
                            updated = True
                        if log_95:
                            section["stats"][key + "_p95"] = (
                                str(
                                    timedelta(
                                        seconds=np.percentile(stats_values, 95)
                                    )
                                )
                                if as_time
                                else float(np.percentile(stats_values, 95))
                            )
                            updated = True
                        if log_min:
                            section["stats"][key + "_min"] = (
                                str(timedelta(seconds=min(stats_values)))
                                if as_time
                                else min(stats_values)
                            )
                            updated = True
                        if log_max:
                            section["stats"][key + "_max"] = (
                                str(timedelta(seconds=max(stats_values)))
                                if as_time
                                else max(stats_values)
                            )
                            updated = True
        return updated
    ...
    def add_sys_info(self):
        ...
        if sys.version_info.major < 3:
            raise NotImplementedError(
                "{}::Requesting system info is not supported for Python 2".format(
        ...
        self.properties["system_info"]["ram"] = (
        ...
        try:
            ...
            if warp.context.runtime is None:
                ...
                print(
                    f"{self.__class__.__name__}::INFO::Saving GPU info -- warp already initialized"
                )
            curr_device = warp.get_device()
            self.properties["system_info"]["GPU"] = (
                curr_device.name if curr_device.is_cuda else "Not used"
            )
        except ImportError:
    ...
    def stats_summary(self):
        ...
        """
        updated_render = self.summarize_stats(
            "render_time", log_sum=True, log_avg=True, as_time=True
        )
        updated_frames = self.summarize_stats("fin_frame", log_avg=True)
        updated_sim_time = self.summarize_stats(
            "sim_time", log_sum=True, log_avg=True, as_time=True
        )
        updated_spf = self.summarize_stats("spf", log_avg=True, as_time=True)
        updated_scan = self.summarize_stats(
            "processing_time", log_sum=True, log_avg=True, as_time=True
        )
        updated_scan_faces = self.summarize_stats(
            "faces_removed", log_avg=True
        )
        ...
        if not (
            updated_frames
            and updated_render
            and updated_sim_time
            and updated_spf
            and updated_self_collisions
            and updated_body_collisions
        ):
            print(
                f"{self.__class__.__name__}::WARNING::Sim stats summary "
    ...
    def _from_file(self, filename):
        ...
        if extention == ".json":
            ...
            raise ValueError(
                f"{self.__class__.__name__}::ERROR::Unsupported file type on load: {extention}"
            )

    def _recursive_dict_update(
        self,
        in_dict,
        new_dict,
        re_write=True,
        ...
        for new_key in new_dict:
            if new_key in in_dict and isinstance(in_dict[new_key], dict):
                ...
                not re_write
                and new_key in in_dict
                and in_dict[new_key] != new_dict[new_key]
            ):
                if in_stats and isinstance(in_dict[new_key], list):
                    ...
                    while (
                        adding_name in in_dict
                    ):  # in case even the added version is already there
