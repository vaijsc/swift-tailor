...
def _camera_surface(
    ...
    dims = top - bottom
    dims = [
        max(dims[0], dims[2]) * ground_scaling_factor,
        dims[1] * vertical_scaling_factor,
    ]

    cube = cmds.polyCube(
        height=dims[1], depth=dims[0], width=dims[0], name="camera_surface"
    )
    ...
    cmds.polyDelFacet(
        cube[0] + ".f[3]"
    )  # we know exact structure of default polyCube in Maya2018 & Maya2020
...
def remove_invisible(target, obstacles=[], num_rays=30, visibile_rays=4):
    ...
    * visibile_rays -- number of rays to hit camera surface without obstacles to consider the face to be visible
    ...
    print(
        "Performing scanning imitation on {} with obstacles {}".format(
            target, obstacles
        )
    )
    ...
    obstacles_accs = [
        mesh.autoUniformGridParams() for mesh in obstacles_meshes
    ]
    to_delete = []

    target_face_iterator = OpenMaya.MItMeshPolygon(target_dag)
    while (
        not target_face_iterator.isDone()
    ):  # https://stackoverflow.com/questions/40422082/how-to-find-face-neighbours-in-maya
        # midpoint of the current face -- start of all the rays
        face_mean = OpenMaya.MFloatPoint(
            target_face_iterator.center(OpenMaya.MSpace.kWorld)
        )
        ...
        for _ in range(num_rays):
            ...
            if (
                utils.test_ray_intersect(
                    camera_surface_mesh,
                    face_mean,
                    rayDir,
                    cam_surface_accelerator,
                )  # intesection with camera surface
                and not any(
                    [
                        utils.test_ray_intersect(
                            mesh,
                        ...
                        for mesh, acc in zip(obstacles_meshes, obstacles_accs)
                    ]
                )  # intesects any of the obstacles
                and not utils.test_ray_intersect(
                    target_mesh,
                    face_mean,
                    rayDir,
                    target_accelerator,
                ...
                visible_count += 1
                if (
                    visible_count >= visibile_rays
                ):  # enough rays are visible -- no need to test more
                    visible = True
    ...
    if len(delete_strs) > 0:
        cmds.polyDelFacet(
            tuple(delete_strs)
        )  # as this is the last command to execute, it could be undone with Ctrl-Z once

    passed = datetime.now() - start_time
    print(
        "{}::Removed {} faces after {}. Press Ctrl-Z to undo the changes".format(
            target, len(to_delete), passed
        )
    )
