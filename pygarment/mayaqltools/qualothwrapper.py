...
def run_sim(garment, props):
    ...
    for frame in range(config["zero_gravity_steps"], config["max_sim_steps"]):
        ...
        _update_progress(frame, config["max_sim_steps"])  # progress bar
        static, non_st_count = garment.is_static(
            config["static_threshold"], config["non_static_percent"]
        )
        if static:  # Success!
    ...
    props["stats"]["spf"][garment.name] = (
        props["stats"]["sim_time"][garment.name] / frame
    )
...
def setFabricProps(cloth, props):
    ...
    cmds.setAttr(
        cloth + ".bendAngleDropOff", props["bend_angle_dropoff"], clamp=True
    )
    cmds.setAttr(cloth + ".bendDamp", props["bend_damp"], clamp=True)
    cmds.setAttr(
        cloth + ".bendDampDropOff", props["bend_damp_dropoff"], clamp=True
    )
    cmds.setAttr(cloth + ".bendYield", props["bend_yield"], clamp=True)
    cmds.setAttr(
        cloth + ".bendPlasticity", props["bend_plasticity"], clamp=True
    )
    ...
    cmds.setAttr(
        cloth + ".compression", props["compression_resistance"], clamp=True
    )

    cmds.setAttr(cloth + ".anisotropicControl", 1)
    cmds.setAttr(
        cloth + ".uStretchScale", props["weft_resistance_scale"], clamp=True
    )
    cmds.setAttr(
        cloth + ".vStretchScale", props["warp_resistance_scale"], clamp=True
    )
...
def fetchColliderFriction(collider_objects):
    ...
    try:
        main_collider = [
            obj for obj in collider_objects if "Offset" not in obj
        ]
        collider_shape = cmds.listRelatives(main_collider[0], shapes=True)
...
def _init_sim(config):
    ...
    # for easy reading of console output
    cmds.setAttr(solver + ".solverStatistics", 0)
    # 0 playback speed = play every frame
    cmds.playbackOptions(ps=0, max=config["max_sim_steps"])
