...
def load_file(filepath, name="object"):
    ...
    if not os.path.isfile(filepath):
        raise RuntimeError(
            "Loading Object from file to Maya::Missing file {}".format(
                filepath
            )
        )
...
def get_mesh_dag(object_name):
    ...
    mesh = OpenMaya.MFnMesh(
        dag
    )  # reference https://help.autodesk.com/view/MAYAUL/2017/ENU/?guid=__py_ref_class_open_maya_1_1_m_fn_mesh_html
...
def match_vert_lists(short_list, long_list):
    ...
    for idx_long in range(len(long_list)):
        ...
        if all(np.isclose(short_vertex, long_vertex, atol=1e-5)):
            ...
            if idx_short >= len(
                short_list
            ):  # short list finished before the long one
                break

    if len(match_list) != len(short_list):
        raise ValueError(
            "Vertex matching unsuccessfull: matched {} of {} vertices in short list".format(
...
def edge_vert_ids(mesh, edge_id):
    ...
    v_ids_cptr = (
        script_util.asInt2Ptr()
    )  # https://forums.cgsociety.org/t/mfnmesh-getedgevertices-error-on-2011/1652362
...
def scale_to_cm(target, max_height_cm=220):
    ...
    * default value of max_height_cm is for meshes of humans
    ...
    if height < max_height_cm * 0.01:  # meters
        ...
        print(
            "WARNING: {} is found to use decimeters as units. Scaled up by 10 for cm".format(
                target
            )
        )
        ...
        print(
            "WARNING: {} is found to use millimiters as units. Scaled down by 0.1 for cm".format(
                target
            )
        )
