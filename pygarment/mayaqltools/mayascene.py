...
from pygarment.pattern.utils import rel_to_abs_2d, vector_angle
...
class MayaGarment(wrappers.VisPattern):
    ...
    # ------ Basic operations ------
    def load(
        self, obstacles=[], shader_group=None, config={}, parent_group=None
    ):
        """
        ...
        if self.is_self_intersecting():
            ...
            raise PatternLoadingError(
                "{}::{}::Provided pattern has self-intersecting panels. Nothing is loaded".format(
    ...
    def stitch_panels(self):
        ...
        for stitch in self.pattern["stitches"]:
            ...
            stitch_id = cmds.parent(
                stitch_id, self.MayaObjects["pattern"]
            )  # organization
    ...
    def setShaderGroup(self, shader_group=None):
        ...
        if self.shader_group is not None:
            cmds.sets(
                self.get_qlcloth_geomentry(), forceElement=self.shader_group
            )
    ...
    def save_mesh(self, folder="", tag="sim"):
        ...
        if not self.loaded_to_maya:
            print(
                "MayaGarmentWARNING::Pattern is not yet loaded. Nothing saved"
            )
            return
    ...
    def clean(self, delete=False):
        ...
        if self.loaded_to_maya:
            ...
            if delete:
                print(
                    "MayaGarment::Deleting {}".format(
                        self.MayaObjects["pattern"]
                    )
                )
                ...
                if (
                    "segm_node" in self.MayaObjects
                    and self.MayaObjects["segm_node"]
                ):
    ...
    def display_vertex_segmentation(self, shader):
        ...
        vertex_select_lists = dict.fromkeys(
            self.panel_order() + ["stitch", "Other"]
        )
        ...
        for vert_idx in range(len(self.current_verts)):
            ...
            vert_addr = "{}.vtx[{}]".format(
                self.get_qlcloth_geomentry(), vert_idx
            )
        ...
        # CRAZY SITUATION  + 0.85 https://www.schemecolor.com/crazy-situation.php
        color_hex = [
            "8594AB",
            "9FBBBA",
            "FACDA7",
        ...
        for idx in range(len(color_hex)):
            color_list[idx] = (
                np.array(
                    [int(color_hex[idx][i : i + 2], 16) for i in (0, 2, 4)]
                )
                / 255.0
            )
        ...
        for label, str_label in enumerate(vertex_select_lists.keys()):
            # 'Other' may not be present at all
            if len(vertex_select_lists[str_label]) > 0:
                if str_label == "Other":  # non-segmented becomes white
                    ...
                    factor, color_id = (
                        label // len(color_list)
                    ) + 1, label % len(color_list)
                    # / factor  # gets darker the more labels there are
                    color = color_list[color_id] * 0.9
        ...
        cmds.connectAttr(
            (data_node + ".outColor"), (shader + ".color")
        )  # Assume Lambert shader
    ...
    def add_colliders(self, obstacles=[]):
        ...
        for obj in self.obstacles:
            collider = qw.qlCreateCollider(self.get_qlcloth_geomentry(), obj)
    ...
    def fetchSimProps(self):
        ...
        self.config["material"] = qw.fetchFabricProps(
        ...
        if "colliders" in self.MayaObjects and self.MayaObjects["colliders"]:
            ...
            friction = qw.fetchColliderFriction(
                self.MayaObjects["colliders"][0]
            )
            if friction:
                self.config["body_friction"] = friction

        self.config["collision_thickness"] = cmds.getAttr(
            self.get_qlcloth_props_obj() + ".thickness"
        )
    ...
    def cache_if_enabled(self, frame):
        ...
        if not self.loaded_to_maya:
            print(
                "MayaGarmentWARNING::Pattern is not yet loaded. Nothing cached"
            )
            return

        if hasattr(self, "cache_path") and self.cache_path:
            self._save_to_path(
                self.cache_path, self.name + "_{:04d}".format(frame)
            )
    ...
    def get_qlcloth_geom_dag(self):
        ...
        if "shapeDAG" not in self.MayaObjects:
            self.MayaObjects["shapeDAG"] = utils.get_dag(
    ...
    def is_static(self, threshold, allowed_non_static_percent=0):
        ...
        # compare vertex-wize to allow accurate control over outliers
        non_static_len = len(diff_L1[diff_L1 > threshold])

        if (
            non_static_len == 0
            or non_static_len
            < len(self.current_verts) * 0.01 * allowed_non_static_percent
        ):
            print(
                "\nStatic with {} non-static vertices out of {}".format(
                    non_static_len, len(self.current_verts)
                )
            )
    ...
    def intersect_colliders_3D(self, obstacles=[]):
        ...
        if not self.loaded_to_maya:
            raise RuntimeError(
                "Garment is not yet loaded: cannot check for intersections"
            )
    ...
    def self_intersect_3D(self, verbose=False):
        ...
        for edge_id in range(num_edges):
            ...
            hit, hitFaces, hitPoints, _ = utils.test_ray_intersect(
                mesh, raySource, rayDir, accelerator, return_info=True
            )
            ...
            for face_id in range(hitFaces.length()):
                ...
                face_verts = [
                    face_verts[j] for j in range(face_verts.length())
                ]
        ...
        if (
            "self_intersect_hit_threshold" in self.config
            and num_hits > self.config["self_intersect_hit_threshold"]
            or num_hits > 0
            and "self_intersect_hit_threshold" not in self.config
        ):  # non-zero hit if no threshold provided
            print(
                "{} is self-intersecting with {} intersect edges -- above threshold {}".format(
                    self.name,
                    num_hits,
                    (
                        self.config["self_intersect_hit_threshold"]
            ...
            print(
                "{} is self-intersecting with {} intersect edges -- ignored by threshold {}".format(
                    self.name,
                    num_hits,
                    (
                        self.config["self_intersect_hit_threshold"]
    ...
    def _load_panel(self, panel_name, pattern_group=None):
        ...
        for edge in panel["edges"]:
            ...
            if (
                ...
                curve = cmds.curve(
                    p=curve_points, d=min(len(curve_points) - 1, 3)
                )
            else:
        ...
        self.MayaObjects["panels"][panel_name]["qlPattern"] = (
            pattern_object[0]
            if panel_group in pattern_object[0]
            else panel_group + "|" + pattern_object[0]
        )
    ...
    def _setSimProps(self, config={}):
        ...
        qw.setFabricProps(
            self.get_qlcloth_props_obj(), self.config["material"]
        )
        ...
        if "collision_thickness" in self.config:
            ...
            cmds.setAttr(
                self.get_qlcloth_props_obj() + ".overrideThickness", 1
            )
            cmds.setAttr(
                self.get_qlcloth_props_obj() + ".thickness",
                self.config["collision_thickness"],
            )
    ...
    def _eval_vertex_segmentation(self):
        ...
        if not self.loaded_to_maya:
            raise RuntimeError(
                "Garment should be loaded when evaluating vertex segmentation"
            )
        ...
        # TODOLOW I can even distinguish stitches from each other!
        ...
        while len(vertices_multi_match) > 0:
            ...
            if len(on_panel_planes) == 1:  # found!
                ...
                neighbors = [
                    ...
                    if vert_id not in unlabelled and vert_id not in on_stitches
                ]

                if len(neighbors) > 0:
                    neighbour_labels = [
                        self.vertex_labels[vert_id] for vert_id in neighbors
                    ]

                    # https://www.geeksforgeeks.org/python-find-most-frequent-element-in-a-list
                    frequent_label = max(
                        set(neighbour_labels), key=neighbour_labels.count
                    )
                    ...
                    print(
                        "Garment::Labelling::vertex {} needs revisit".format(
                            unlabeled_vert_id
                        )
                    )
                    vertices_multi_match.append(
                        (unlabeled_vert_id, on_panel_planes)
                    )
    ...
    def _clean_mesh(self):
        ...
        # fix labeling
        self.update_verts_info()
        match_verts = utils.match_vert_lists(
            self.current_verts, self.last_verts
        )
    ...
    def _edge_as_3d_tuple_list(self, edge, vertices):
        ...
        if "curvature" in edge:
            if isinstance(edge["curvature"], list):
                ...
                points = np.r_[[points[0]], abs_points, [points[1]]]

            elif (
                edge["curvature"]["type"] == "cubic"
                or edge["curvature"]["type"] == "quadratic"
            ):
                abs_points = []
                for p in edge["curvature"]["params"]:
                    abs_points.append(rel_to_abs_2d(points[0], points[1], p))
                points = np.r_[[points[0]], abs_points, [points[1]]]
            else:
    ...
    def _draw_circle_arc(self, edge, vertices, resolution=20):
        ...
        angle = 2 * np.arcsin(
            np.linalg.norm(edge_3d[1] - edge_3d[0]) / radius / 2
        )
        if large_arc:
            angle = 2 * np.pi - angle
        ...
        sec_point = np.array(
            [-radius * np.sin(angle), radius * np.cos(angle), 0]
        )
    ...
    def _applyEuler(self, vector, eulerRot):
        ...
        vector_x[1] = vector[1] * np.cos(eulerRot_rad[0]) - vector[2] * np.sin(
            eulerRot_rad[0]
        )
        vector_x[2] = vector[1] * np.sin(eulerRot_rad[0]) + vector[2] * np.cos(
            eulerRot_rad[0]
        )

        # Y
        vector_y = np.copy(vector_x)
        vector_y[0] = vector_x[0] * np.cos(eulerRot_rad[1]) + vector_x[
            2
        ] * np.sin(eulerRot_rad[1])
        vector_y[2] = -vector_x[0] * np.sin(eulerRot_rad[1]) + vector_x[
            2
        ] * np.cos(eulerRot_rad[1])

        # Z
        vector_z = np.copy(vector_y)
        vector_z[0] = vector_y[0] * np.cos(eulerRot_rad[2]) - vector_y[
            1
        ] * np.sin(eulerRot_rad[2])
        vector_z[1] = vector_y[0] * np.sin(eulerRot_rad[2]) + vector_y[
            1
        ] * np.cos(eulerRot_rad[2])

        return vector_z

    def _set_panel_3D_attr(
        self, panel_dict, panel_group, attribute, maya_attr
    ):
    ...
    def _save_to_path(self, path, filename):
        ...
        if num_verts != len(self.vertex_labels):
            print(
                "MayaGarment::WARNING::Segmentation list does not match mesh topology in save {}".format(
                    self.name
                )
            )
    ...
    def _intersect_object(self, geometry):
        ...
        cloth_mesh, cloth_dag = utils.get_mesh_dag(
            self.get_qlcloth_geomentry()
        )
        ...
        # those are edges along the border of intersecting area on the geometry
        hit_border_length = 0
        for edge_id in range(num_edges):
            # Vertices that comprise an edge
            ...
            rayDir = OpenMaya.MFloatVector(
                obs_vertices[vtx2] - obs_vertices[vtx1]
            )
            hit = utils.test_ray_intersect(
                cloth_mesh, raySource, rayDir, accelerator
            )
            if hit:
        ...
        if (
            hit_border_length < 1e-5
        ):  # no intersections -- no need for threshold check
            print("{} with {} do not intersect".format(geometry, self.name))
            return False

        if (
            "object_intersect_border_threshold" in self.config
            and hit_border_length
            > self.config["object_intersect_border_threshold"]
            or (
                hit_border_length > 1e-5
                and "object_intersect_border_threshold" not in self.config
            )
        ):  # non-zero hit if no threshold provided
            print(
                "{} with {} intersect::Approximate intersection border length {:.2f} cm is above threshold {:.2f} cm".format(
                    geometry,
                    self.name,
                    hit_border_length,
                    (
        ...
        print(
            "{} with {} intersect::Approximate intersection border length {:.2f} cm is ignored by threshold {:.2f} cm".format(
                geometry,
                self.name,
                hit_border_length,
                (
    ...
    def _verts_on_stitches(self):
        ...
        for stitch in self.pattern["stitches"]:
            ...
            for side in [0, 1]:
                ...
                panel_node = self.MayaObjects["panels"][panel_name][
                    "qlPattern"
                ]
    ...
    def _all_panel_bboxes(self):
        ...
        for panel in panel_curves:
            box = cmds.exactWorldBoundingBox(
                panel_curves[panel]["curve_group"]
            )
            bboxes[panel] = box
        return bboxes
    ...
    def _point_in_bbox(point, bbox, tol=0.01):
        ...
        if (
            point[0] < (bbox[0] - tol)
            or point[0] > (bbox[3] + tol)
            or point[1] < (bbox[1] - tol)
            or point[1] > (bbox[4] + tol)
            or point[2] < (bbox[2] - tol)
            or point[2] > (bbox[5] + tol)
        ):
            return False
        return True
    ...
    def _point_on_plane(self, point, panel, tol=0.001):
        ...
        return np.isclose(dot_prod, 0.0, atol=tol)
    ...
    def _get_vert_neighbours(self, vert_id):
        ...
        edges = cmds.polyListComponentConversion(
            mesh_name + ".vtx[%d]" % vert_id, fromVertex=True, toEdge=True
        )

        neighbors = []
        for edge in edges:
            neighbor_verts_str = cmds.polyListComponentConversion(
                edge, toVertex=True
            )
...
class MayaGarmentWithUI(MayaGarment):
    ...
    def drawUI(self, top_layout=None):
        ...
        if not self.loaded_to_maya:
            ...
            for panel in self.pattern["panels"]:
                panel_layout = cmds.frameLayout(
                    ...
                    mh=10,
                    mw=10,
                    expandCommand=partial(
                        cmds.select,
                        self.MayaObjects["panels"][panel]["curve_group"],
                    ),
                    collapseCommand=partial(
                        cmds.select,
                        self.MayaObjects["panels"][panel]["curve_group"],
                    ),
                )
        ...
        if "constraints" in self.spec:
            ...
            self._ui_constraints(
                self.spec["constraints"], self.spec["constraint_order"]
            )
            cmds.setParent("..")
    ...
    def _ui_3d_placement(self, panel_name):
        ...
        # Position
        cmds.attrControlGrp(
            attribute=self.MayaObjects["panels"][panel_name]["curve_group"]
            + ".translate",
            changeCommand=partial(
                self._panel_placement_callback,
                panel_name,
                "translation",
                "translate",
            ),
    ...
    def _ui_param_value(
        self, param_name, param_range, value, idx=None, tag=""
    ):
        ...
        value_field = cmds.floatSliderGrp(
            ...
            field=True,
            value=value,
            minValue=param_range[0],
            maxValue=param_range[1],
            cal=[1, "left"],
            cw=[1, 45],
            step=0.01,
        )
        ...
        cmds.floatSliderGrp(
            ...
            edit=True,
            changeCommand=partial(
                self._param_value_callback, param_name, idx, value_field
            ),
        )
    ...
    def _ui_params(self, params, order):
        ...
        for param_name in order:
            ...
            if isinstance(values, list):
                ui_tags = ["X", "Y", "Z", "W"]
                for idx, (value, param_range) in enumerate(
                    zip(values, param_ranges)
                ):
                    self._ui_param_value(
                        param_name, param_range, value, idx, ui_tags[idx]
                    )
            else:
    ...
    def _ui_constraints(self, constraints, order):
        ...
        for constraint_name in order:
            cmds.textFieldGrp(
                ...
                text=constraints[constraint_name]["type"],
                editable=False,
                cal=[1, "left"],
                cw=[1, 90],
            )
    ...
    def _param_value_callback(self, param_name, value_idx, value_field, *args):
        ...
        if self.is_self_intersecting():
            result = cmds.confirmDialog(
                ...
                message=(
                    "WARNING: Some of the panels contain intersected edges after applying value {} to {}."
                    "\nDo you want to revert to previous state?"
                    "\n\nNote: simulation in broken state might result in Maya crashing"
                ).format(new_value, param_name),
                button=["Yes", "No"],
                defaultButton="Yes",
                cancelButton="No",
                dismissString="No",
...
class Scene(object):
    ...
    def __init__(self, body_obj, props, scenes_path="", clean_on_die=False):
        ...
        if "scene" in self.config:
            self._load_maya_scene(
                os.path.join(scenes_path, self.config["scene"])
            )
        else:
    ...
    def _init_arnold(self):
        ...
        # turn on GPPU rendering
        cmds.setAttr("defaultArnoldRenderOptions.renderDevice", 1)
        # switch to CPU in case of failure
        cmds.setAttr("defaultArnoldRenderOptions.render_device_fallback", 1)
        # increase sampling for clean results
    ...
    def render(self, save_to, name="last"):
        ...
        # image setup
        old_setup = self._set_image_size(
            im_size, im_size[0] / im_size[1], im_size[0] / im_size[1]
        )
        ...
        cmds.colorManagementPrefs(
            e=True, outputTransformEnabled=True, outputUseViewTransform=True
        )
        ...
        for camera in self.cameras:
            ...
            # list of one element if ':' is not found
    ...
    def fetch_props_from_Maya(self):
        ...
        self.config["garment_color"] = self._fetch_color(
            self.scene["cloth_shader"]
        )
    ...
    def reset_garment_color(self):
        ...
        if cmds.connectionInfo(color_plug, isDestination=True):
            connected_color_plug = cmds.connectionInfo(
                color_plug, sourceFromDestination=True
            )
        ...
        cmds.setAttr(
            (color_plug), color[0], color[1], color[2], type="double3"
        )
    ...
    def _fetch_color(self, shader):
        ...
        if cmds.connectionInfo(shader + ".color", isDestination=True):
            color_plug = cmds.connectionInfo(
                color_plug, sourceFromDestination=True
            )
    ...
    def _simple_scene_setup(self):
        ...
        colors = {
            ...
            "cloth_color": (
                [0.8, 0.2, 0.2]
                if "garment_color" not in self.config
                else self.config["garment_color"]
            ),
            "floor_color": [0.8, 0.8, 0.8],
        }

        self.scene = {"floor": self._add_floor(self.body)}
        # materials
        self.scene["body_shader"], self.scene["body_SG"] = self._new_lambert(
            colors["body_color"], self.body
        )
        self.scene["cloth_shader"], self.scene["cloth_SG"] = self._new_lambert(
            colors["cloth_color"], self.body
        )
        self.scene["floor_shader"], self.scene["floor_SG"] = self._new_lambert(
            colors["floor_color"], self.body
        )

        self.scene["light"] = mutils.createLocator(
    ...
    def _load_maya_scene(self, scenefile):
        ...
        self.scene = {
            "scene_group": cmds.ls(
                scene_namespace + "*scene*", transforms=True
            )[0],
            "floor": cmds.ls(scene_namespace + "*floor*", geometry=True)[0],
            "body_shader": cmds.ls(scene_namespace + "*body*", materials=True)[
                0
            ],
            "cloth_shader": cmds.ls(
                scene_namespace + "*garment*",
                materials=True,
            )[0],
        }
        # shader groups (to be used in cmds.sets())
        self.scene["body_SG"] = self._create_shader_group(
            self.scene["body_shader"], "bodySG"
        )
        self.scene["cloth_SG"] = self._create_shader_group(
            self.scene["cloth_shader"], "garmentSG"
        )
        ...
        cmds.setAttr(
            ...
            type="double3",
        )  # apply to whole group s.t. lights positions were adjusted too
    ...
    def _add_simple_camera(self, rotation=[-23.2, 16, 0]):
        ...
        camera = cmds.camera(
            aspectRatio=self.config["resolution"][0]
    ...
    def _get_object_lower_center(self, object):
        ...
        return [(bb[3] + bb[0]) / 2, bb[1], (bb[5] + bb[2]) / 2]
    ...
    def _add_floor(self, target):
        ...
        cmds.move(
            (target_bb[3] + target_bb[0]) / 2,  # bbox center
            floor_level,
    ...
    def _new_lambert(self, color, target=None):
        ...
        cmds.setAttr(
            (shader + ".color"), color[0], color[1], color[2], type="double3"
        )
    ...
    def _create_shader_group(self, material, name="shader"):
        ...
        cmds.connectAttr(
            material + ".outColor", shader_group + ".surfaceShader"
        )
        return shader_group
