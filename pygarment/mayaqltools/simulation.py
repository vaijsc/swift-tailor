...
def single_file_sim(resources, props, caching=False):
    ...
    try:
        ...
        # Main part
        template_simulation(
            os.path.join(resources["templates_path"], props["templates"]),
            scene,
            props["sim"],
            caching=caching,
        )
        ...
        try:
            ...
            pass
        props.serialize(
            os.path.join(resources["templates_path"], "props.json")
        )
...
def init_sim_props(props, batch_run=False, force_restart=False):
    ...
    if "render" not in props:
        # init with defaults
        props.set_section_config("render", resolution=[800, 800])
...
def template_simulation(
    ...
    try:
        ...
        if save_maya_scene:
            ...
            cmds.file(
                rename=os.path.join(garment.path, garment.name + "_scene")
            )
            cmds.file(
                save=True,
                type="mayaBinary",
        ...
        for directory in dirs:
            if directory not in to_ignore:
                ...
                pattern_specs.append(
                    os.path.join(root, directory, "specification.json")
                )
    else:
        for file in files:
