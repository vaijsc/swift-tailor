...
def start_GUI():
    ...
    # body load
    text_button_group(
        load_body_callback, state, label="Body file: ", button_label="Load"
    )
    # props load
    text_button_group(
        load_props_callback, state, label="Properties: ", button_label="Load"
    )
    # scene setup load
    text_button_group(
        load_scene_callback, state, label="Scene: ", button_label="Load"
    )
    # separate
    cmds.separator()
    ...
    # Saving folder
    saving_to_field = text_button_group(
        saving_folder_callback,
        state,
        label="Saving to: ",
        button_label="Choose",
    )
    ...
    cmds.button(
        label="Save with render",
        backgroundColor=[255 / 256, 140 / 256, 73 / 256],
...
class State(object):
    def __init__(self):
        ...
        # use default setup for simulation -- for now
        mymaya.simulation.init_sim_props(self.config)
...
def template_field_callback(view_field, state, *args):
    ...
    current_dir = os.path.dirname(
        cmds.textField(view_field, query=True, text=True)
    )
...
def load_body_callback(view_field, state, *args):
    ...
    # previous scene will autodelete
...
def load_props_callback(view_field, state, *args):
    ...
    if "scene" not in state.config["render"]["config"]:  # remove entirely
        ...
        in_config["render"]["config"]["scene"] = state.config["render"][
            "config"
        ]["scene"]
...
def start_sim_callback(button, state, *args):
    ...
    if state.garment is None or state.scene is None:
        cmds.confirmDialog(
            title="Error",
            message="Load pattern specification & body info first",
        )
        return
    print("Simulating..")
    ...
    cmds.button(
        ...
        edit=True,
        label="Stop Sim",
        backgroundColor=[245 / 256, 96 / 256, 66 / 256],
...
def check_collisions_callback(button, state, *args):
    ...
    cmds.confirmDialog(
        ...
        message=(
            ...
            "Garment has self-intersections: {}"
        ).format(
    ...
    cmds.button(
        ...
        edit=True,
        label="Collisions",
        backgroundColor=[250 / 256, 200 / 256, 119 / 256],
...
def imitate_3D_scan_callback(button, state, *args):
    ...
    cmds.button(
        ...
        edit=True,
        label="3D Scan",
        backgroundColor=[200 / 256, 225 / 256, 80 / 256],
...
def display_segmentation_callback(button, state, *args):
    ...
    if not state.segmented:
        ...
        state.garment.display_vertex_segmentation(
            state.scene.scene["cloth_shader"]
        )
        ...
        cmds.button(
            ...
            edit=True,
            label="Color",
            backgroundColor=[150 / 256, 225 / 256, 80 / 256],
...
def _create_saving_dir(view_field, state):
    ...
    if state.garment is None:
        cmds.confirmDialog(
            title="Error", message="Load pattern specification first"
        )
