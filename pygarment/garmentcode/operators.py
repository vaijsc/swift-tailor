...
def cut_corner(
    ...
    * target_interface -- the chained pairs of edges that form the corner
    ...
    if verbose and not out.success:
        print(
            f"Cut_corner::ERROR::finding the projection (translation) is unsuccessful. Likely an error in edges choice"
        )
        print(out)
...
def cut_into_edge(
    ...
    * Edges that lie on the original base edge
    ...
    if flip_target:
        ...
        # Flip
        target_shape = [
            s.reflect([0, median_y], [1, median_y]) for s in target_shape
        ]
...
def cut_into_edge_single(
    ...
    * Edges that lie on the original base edge
    ...
    if (
        offset < target_shape_w / 2 - tol
        or offset > (edge_len - target_shape_w / 2) + tol
    ):
    ...
    if verbose and not out.success:
        print(
            f"Cut_edge::ERROR::finding the projection (translation) is unsuccessful. Likely an error in edges choice"
        )
    ...
    ins_point = (
        c_to_np(curve.point(rel_offset - shift[1]))
        if (rel_offset - shift[1]) > tol
        else base_edge.start
    )
    fin_point = (
        c_to_np(curve.point(rel_offset + shift[0]))
        if (rel_offset + shift[0]) < 1 - tol
        else base_edge.end
    )
    ...
    right_position = (
        np.sign(
            np.cross(
                insert_vector, avg_vertex - np.asarray(new_edges[0].start)
            )
        )
        == -1
    )
    ...
    if ins_point is base_edge.start:
        ...
        start_part = base_edge.subdivide_param(
            [rel_offset - shift[1], 1 - (rel_offset - shift[1])]
        )[0]
    ...
    if fin_point is base_edge.end:
        ...
        end_part = base_edge.subdivide_param(
            [rel_offset + shift[0], 1 - (rel_offset + shift[0])]
        )[-1]
...
def _fit_location_corner(
    l, diff_target, curve1, curve2, verbose: bool = False
):
    ...
    return (diff_curr[0] - diff_target[0]) ** 2 + (
        diff_curr[1] - diff_target[1]
    ) ** 2


def _fit_location_edge(
    shift, location, width_target, curve, verbose: bool = False
):
    """Find the points on two curves s.t. vector between them is the same as
    shortcut"""
    ...
    if verbose:
        print(
            "Location Progression: ",
            (_dist(point1, point2) - width_target) ** 2,
        )
...
def even_armhole_openings(
    front_opening, back_opening, tol=1e-2, verbose: bool = False
):
    """
    ...
    if len(intersect_t) >= 1 and not (
        close_enough(
            intersect_t[0][0], 0, tol=tol
        )  # checking if they are already ok separated
        ...
        subdiv[1].start, subdiv[1].end = copy(subdiv[1].start), copy(
            subdiv[1].end
        )  # Disconnect vertices in subdivided version
...
def _bend_extend_2_tangent(
    ...
    tan_0_diff = (
        abs(curve_inverse.unit_tangent(0) - target_tangent_start)
    ) ** 2
    ...
    curvature_reg = (
        _max_curvature(curve_inverse, points_estimates=point_estimates) ** 2
    )
...
def curve_match_tangents(
    curve,
    target_tan0,
    target_tan1,
    target_len=None,
    return_as_edge=False,
    verbose: bool = False,
):
    ...
    * Or CurveEdge instance, if return_as_edge=True
    ...
    if not out.success:
        if verbose:
            print(
                f"Curve_match_tangents::WARNING::optimization not successfull"
            )
            print(out)
...
def _fit_scale(s, shortcut, v1, v2, vc, d_v1, d_v2):
    ...
    return (d_v1 - _dist(shifted[0], v1) - _dist(shifted[0], vc)) ** 2 + (
        d_v2 - _dist(shifted[1], v2) - _dist(shifted[1], vc)
    ) ** 2
