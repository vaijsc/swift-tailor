...
class BodyParametrizationBase:
    ...
    def save(self, path, name="body_measurements"):
        with open(Path(path) / f"{name}.yaml", "w") as f:
            yaml.dump({"body": self.params}, f, default_flow_style=False)
...
class DesignSampler:
    ...
    def _randomize_value(self, random_params, path):
        ...
        try:
            ...
            KeyError
        ) as e:  # Default probability not given  -> Sample uniformly
            def_prob = None
        ...
        if self.__use_default(def_prob):
            ...
            if (
                "select" in p_type or p_type == "bool" or "file" in p_type
            ):  # All discrete types
                ...
                new_val = self.__randint_exclude(
                    range, None if def_prob is None else def_value
                )
            elif p_type == "float":
                new_val = self.__uniform_exclude(
                    range, None if def_prob is None else def_value
                )
