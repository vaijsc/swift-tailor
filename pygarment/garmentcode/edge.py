...
class CircleEdge(Edge):
    ...
    def _subdivide(self, fractions: list, by_length=False):
        ...
        from pygarment.garmentcode.edge_factory import (  # TODOLOW: ami - better solution?
            EdgeFactory,
        )
...
class CurveEdge(Edge):
    ...
    def as_curve(self, absolute=True):
        ...
        if absolute:
            cp = [
                rel_to_abs_2d(self.start, self.end, c)
                for c in self.control_points
            ]
...
class EdgeSequence:
    ...
    def extend(self, factor):
        ...
        target_line = np.array(chained_edges[-1].end) - np.array(
            chained_edges[0].start
        )
