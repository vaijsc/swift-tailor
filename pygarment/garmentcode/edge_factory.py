...
class CircleEdgeFactory:
    ...
    def from_points_angle(start, end, arc_angle, right=True):
        ...
        control_y = (
            radius + h if to_sum else radius - h
...
class CurveEdgeFactory:
    ...
    def curve_3_points(start, end, target, verbose=False):
        ...
        out = minimize(_fit_pass_point, rel_target, args=(rel_target))

        if not out.success:
            if verbose:
                print(
                    "Curve From Extreme::WARNING::Optimization not successful"
                )
                print(out)
...
class EdgeSeqFactory:
    ...
    def dart_shape(width, side_len=None, depth=None):
        ...
        return EdgeSeqFactory.from_verts(
            [0, 0], [width / 2, -depth], [width, 0]
        )
    ...
    def halfs_from_svg(svg_filepath, target_height=None):
        ...
        * target_height -- scales the shape s.t. it's height matches the given
        ...
        left_seqs = [
            p.reflect([bbox[0], center_y], [bbox[1], center_y])
            for p in left_seqs
        ]
        right_seqs = [
            p.reflect([bbox[0], center_y], [bbox[1], center_y])
...
def _fit_pass_point(cp, target_location):
    ...
    * target_location -- target to fit extremum to --
    ...
    """
    control_bezier = np.array([[0, 0], cp, [1, 0]])
...
def _fit_tangents(
    cp, target_tangent_start, target_tangent_end, reg_strength=0.01
):
    """Fit the control point of basic [[0, 0] -> [1, 0]] Quadratic Bezier s.t.
...
def split_half_svg_paths(paths):
    ...
    # Mid-Intersection
    inter_segment = svgpath.Line(
        center_x + 1j * bbox[2], center_x + 1j * bbox[3]
    )
    ...
    for p in paths:
        ...
        side_2 = svgpath.Path(
            *p.cropped(to_T, 1)._segments, *p.cropped(0, from_T)._segments
        )
