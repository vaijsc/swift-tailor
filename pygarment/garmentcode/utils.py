...
def vector_align_3D(v1, v2):
    ...
    v1, v2 = np.asarray(v1), np.asarray(v2)
    cos = np.dot(v1, v2) / (norm(v1) * norm(v2))
    # NOTE: getting rid of numbers like 1.000002 that appear due to numerical instability
    cos = max(min(cos, 1), -1)
...
def lin_interpolation(val1, val2, factor):
    ...
    if factor < 0 or factor > 1:
        raise ValueError(
            f"lin_interpolation::ERROR::Expected a factor \in [0, 1], got {factor}"
        )
...
def curve_extreme_points(curve, on_x=False, on_y=True):
    ...
    if on_x:
        x = svgpath.real(poly)
        dx = x.deriv()
        x_extremizers = svgpath.polyroots(
            dx, realroots=True, condition=lambda r: 0 < r < 1
        )
    all_extremizers = x_extremizers + y_extremizers

    extreme_points = np.array(
        [c_to_list(curve.point(t)) for t in all_extremizers]
    )
