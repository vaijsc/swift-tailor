...
class Interface:
    ...
    def __init__(self, panel, edges, ruffle=1.0, right_wrong=False):
        """
        Parameters:
            * panel - Panel object
        ...
        self.edges = (
            edges if isinstance(edges, EdgeSequence) else EdgeSequence(edges)
        )
        self.panel = [
            panel for _ in range(len(self.edges))
        ]  # matches every edge
    ...
    def projecting_edges(self, on_oriented=False) -> EdgeSequence:
        ...
        projected = (
            self.edges.copy() if not on_oriented else self.oriented_edges()
        )
        for r in self.ruffle:
            if not close_enough(r["coeff"], 1, 1e-3):
                if (
                    r["sec"][1] >= len(projected)
                    or not projected[
                        r["sec"][1] - 1 : r["sec"][1] + 1
                    ].isChained()
                ):
                    ...
                    common_v = (
                        prev_edge.end
                        if prev_edge.end is next_edge.end
                        or prev_edge.end is next_edge.start
                        else prev_edge.start
                    )
                    ...
                    projected[r["sec"][1] :].translate_by(
                        [
                            common_v[0] - common_v_copy[0],
                            common_v[1] - common_v_copy[1],
                        ]
                    )
    ...
    def projecting_lengths(self):
        ...
        for r in self.ruffle:
            if not close_enough(r["coeff"], 1, 1e-3):
                ...
                projecting_lengths += [
                    e.length() for e in self.edges[r["sec"][0] : r["sec"][1]]
                ]
    ...
    def reorder(self, curr_edge_ids, projected_edge_ids):
        ...
        for i in range(len(self.panel)):
            id = (
                i
                if i not in curr_edge_ids
                else projected_edge_ids[curr_edge_ids.index(i)]
            )
            # edges
    ...
    def from_multiple(*ints):
        ...
        * different ruffle values in one interface
        ...
        new_int = copy(
            ints[0]
        )  # shallow copy -- don't create unnecessary objects
    ...
    @staticmethod
    def _is_order_matching(
        panel_s, vert_s, panel_1, vert1, panel_2, vert2
    ) -> bool:
