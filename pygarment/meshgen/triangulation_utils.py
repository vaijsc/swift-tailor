...
from CGAL.CGAL_Mesh_2 import (
    ...
    Mesh_2_Constrained_Delaunay_triangulation_2,
)
...
def get_edge_vert_ids(edges):
    ...
    for edge in edges:
        ...
        zipped_array = np.concatenate(
            (zipped_array, zipped_array_edge), axis=0
        )
...
def cdt_insert_constraints(cdt, cdt_points, edge_verts_ids):
    ...
    # [id into cdt.finite_vertices()] -> [replace by this id into cdt.finite_vertices()]
    new_points = {}
    ...
    for s_id, e_id in edge_verts_ids:
        ...
        if init_len != num_verts:
            ...
            init_len = num_verts
            print(
                "triangulation_utils::INFO::Generated extra boundary points for sdt contraints. Postprocessing will be performed"
            )
        ...
        if face_info_new[face].in_domain():
            ...
            if new_points:
                ...
                if not (
                    ...
                    sorted_faces
                    and np.any(
                        np.all(np.array(sorted_faces) == sorted(v_ids), axis=1)
                    )
                ):
...
def get_keep_vertices(cdt, len_b):
    ...
    unique_edges, counts = np.unique(
        np.array(edges), axis=0, return_counts=True
    )
