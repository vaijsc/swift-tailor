...
def optimize_garment_storage(paths: PathCofig):
    """Prepare the data element for compact storage: store the meshes as ply
    instead of obj, remove texture files
    """
    # Objs to ply
    try:
...
def sim_frame_sequence(garment, config, store_usd=False, verbose=False):
    ...
    for frame in range(0, config.max_sim_steps):
        ...
        if (
            frame >= config.zero_gravity_steps
            and frame >= config.min_sim_steps
        ):
...
def run_sim(
    ...
    try:
        ...
        ):  # 0.5 sec  -- finished suspiciously fast
        ...
        if num_self_collisions:
            print(
                f"Self-Intersecting with {num_self_collisions}, "
    ...
    sim_props["stats"]["sim_time"][cloth_name] = sim_time = (
        time.time() - start_time
    )
    sim_props["stats"]["spf"][cloth_name] = (
        sim_time / frame if frame else sim_time
    )
