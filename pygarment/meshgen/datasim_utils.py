...
def batch_sim(
    ...
    resume = init_sim_props(
        dataset_props, batch_run=True, force_restart=force_restart
    )
    ...
    for pattern_name in pattern_names:
        ...
        if (
            resume
            and pattern_name in dataset_props["sim"]["stats"]["processed"]
        ):
            print(f"Skipped as already processed {pattern_name}")
            continue

        dataset_props["sim"]["stats"]["processed"].append(pattern_name)
        _serialize_props_with_sim_stats(
            dataset_props, data_props_file
        ...
        if (
            num_samples is not None and count >= num_samples
        ):  # only process requested number of samples
            break

    # Fin
    print(f"\nFinished batch of {data_path}")
    try:
        if len(dataset_props["sim"]["stats"]["processed"]) >= len(
            pattern_names
        ):
...
def resim_fails(
    ...
    for key in fails:
        if key not in ["cloth_body_intersection", "cloth_self_intersection"]:
            for el in fails[key]:
                to_resim.add(el)
            # NOTE: If nothing to be added in this key, it was already an empty array (and nothing changed)
            fails[key] = []
...
def init_sim_props(props: Properties, batch_run=False, force_restart=False):
    """
    ...
    if "options" not in props["sim"]["config"]:
        props["sim"]["config"]["options"] = {
            ...
            "attachment_stiffness": [1000.0],
            "attachment_damping": [10.0],
    ...
    if (
        batch_run
        and "processed" in props["sim"]["stats"]
        ...
        if not any(
            [
                (name in last_processed) or (last_processed in name)
                for name in props["render"]["stats"]["render_time"]
            ]
        ):
        ...
        props["sim"]["stats"]["stop_over"].append(
            last_processed
        )  # indicate resuming dataset simulation
...
def template_simulation(paths: PathCofig, props, caching=False):
    ...
    timeout_after = int(
        get_dict_default_value(sim_props["config"], "max_meshgen_time", 20)
    )
    ...
    try:
        ...
        sim_props["stats"]["meshgen_time"][garment.name] = (
            time.time() - meshgen_start_time
        )
        sim_props["stats"]["face_count"][garment.name] = len(garment.faces)
        sim_props_option = sim_props["config"]["options"]

        vertex_normals = get_dict_default_value(
            sim_props_option, "store_vertex_normals", False
        )
        store_panels = get_dict_default_value(
            sim_props_option, "store_panels", False
        )
        garment.serialize(
            paths,
...
def _load_boxmesh_timeout(garment, timeout_after):
    if platform.system() == "Windows":
        ...
        p = multiprocessing.Process(
            target=garment.load(), name="GarmentGeneration"
        )
        p.start()
