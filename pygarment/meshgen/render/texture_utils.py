...
def texture_mesh_islands(
    ...
    all_uvs, boundary_uv_to_draw = unwarp_UV(
        texture_coords, face_texture_coords, padding=uv_padding
    )

    # NOTE !! Axis padding should match the uv padding
    uv_list, width, height = normalize_UVs(all_uvs, axis_padding=uv_padding)
    ...
    if out_mtl_file_path:
        save_texture_mtl(
            out_mtl_file_path,
            (
                out_fabric_tex_image_path.name
                if out_fabric_tex_image_path is not None
                else out_texture_image_path.name
            ),
            mat_name=mat_name,
        )
...
def unwarp_UV(texture_coords, face_texture_coords, padding=3):
    ...
    for i in range(num_ccs):
        ...
        verts_translated_bound = [
            (x + translate_X, y + translate_Y) for x, y in bound_vert_pos
        ]
        boundary_uv_to_draw.append(verts_translated_bound)

        # translate all positions
        verts_translated = [
            (x + translate_X, y + translate_Y) for x, y in all_vert_pos
        ]
...
def create_UV_island_texture(
    ...
    # Figure size
    fig, ax = plt.subplots()
    # width & height are usually given in cm
    fig.set_size_inches(width / 100, height / 100)

    # Colors
    shift = 0.17
    divisor = max(5, n_components)
    # copper cool  spring winter twilight  # Using smooth Matplotlib colormaps
    ...
    if background_img_path is not None:
        ...
        ax.imshow(
            back_img[
                : int(width * back_crop_scale),
                : int(height * back_crop_scale),
                :,
            ],
    ...
    for i in range(n_components):
        ...
        # Alpha - transparency for blending with backround
        color[-1] = color_alpha
        ...
        plt.fill(
            ...
            linestyle="-",
            linewidth=boundary_width / 2,  # Boundary stylings
        )
    ...
    with open(output_file_path, "w") as f:
        ...
        if vert_normals is not None:
            for (
                ...
                v_id2,
                tex_id2,
            ) in faces_with_texture:
                f.write(
                    f"f {v_id0 + 1}/{tex_id0 + 1}/{v_id0 + 1} "
            ...
            for (
                ...
                f.write(
                    f"f {v_id0 + 1}/{tex_id0 + 1} "
                    f"{v_id1 + 1}/{tex_id1 + 1} "
                    f"{v_id2 + 1}/{tex_id2 + 1}\n"
                )


def add_texture_to_obj(
    obj_file_path, output_file_path, uv_list, mtl_file_name, mat_name
):
    # Update OBJ-----------------------------------------------------
