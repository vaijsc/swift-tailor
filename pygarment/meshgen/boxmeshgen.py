...
from pygarment.meshgen.render.texture_utils import (
    save_obj,
...
class Panel:
    ...
    def _verts(self, lin_edges):
        ...
        for e in lin_edges:
            ...
            if not np.array_equal(e[0], verts[-1]):
                verts.append(e[0])
            verts.append(e[1])
        # don't double count the loop origin
    ...
    def _bbox(self, verts_2d):
        ...
        if len(b_points) == 2:
            ...
            raise PatternLoadingError(
                "Less than two vertices defining bounding box"
            )
    ...
    def plot(self, pts, title):
        ...
        y_values = pts.T[1]
        plt.scatter(
            x_values, y_values, c="blue", marker="o", label="Data Points"
        )
    ...
    def rot_trans_panel(self, vertices):
        ...
        r_t_vertices = np.vstack(
            tuple(
                [
                    BoxMesh._point_in_3D(v, rot_matrix, self.translation)
    ...
    def gen_panel_mesh(self, mesh_resolution, plot=False, check=False):
        ...
        tri_utils.cdt_insert_constraints(
            cdt_mesh, cdt_points_mesh, edge_verts_ids
        )
        ...
        tri_utils.CGAL_Mesh_2.refine_Delaunay_mesh_2(
            cdt_mesh,
            tri_utils.Delaunay_mesh_size_criteria_2(
                0.125, 1.43 * mesh_resolution
            ),
        )  # 1.475
        ...
        new_points = tri_utils.cdt_insert_constraints(
            cdt, cdt_points, edge_verts_ids
        )
...
class Edge:
    ...
    def init_curve(self, edge, mesh_resolution):
        ...
        if "curvature" in edge:
            # NOTE: placeholder for old curves for backward compatibility
            if (
                ...
                control_point = pat_utils.rel_to_abs_2d(
                ...
                for p in edge["curvature"]["params"]:
                    ...
                    # MYNOTE: convert from relative to absolute
        ...
        if n_edge_verts == 2 and res > 1.0:
            print(
                f"{self.__class__.__name__}::{self.name}::WARNING::Detected edge represented only by two vertices.."
                "mesh resolution might be too low. resolution = {}, edge length = {}".format(
                    res, edgelength
                )
            )
    ...
    def set_vertex_range(self, start_idx, begin_in, end_in, end_idx):
        ...
        """
        self.vertex_range = (
            [start_idx] + list(range(begin_in, end_in)) + [end_idx]
        )
...
class Seam:
    def __init__(
        ...
        """
        # MYNOTE: only for storing values only
...
class BoxMesh(wrappers.VisPattern):
    ...
    def __init__(self, path, res=1.0):
        ...
        self.panelNames: List[str] = self.panel_order()
        self.vertices = []
        self.faces = []
        self.orig_lens = {}
    ...
    def load(self):
        ...
        if self.is_self_intersecting():
            print(
                f"{self.__class__.__name__}::WARNING::{self.name}::Provided pattern has self-intersecting panels. Simulation might crash"
            )
    ...
    def load_panels(self):
        ...
        for panel_name in self.panelNames:
            panel = Panel(
                all_panels[panel_name], panel_name, self.mesh_resolution
            )
    ...
    def _get_stitch_edge_info(
        ...
        try:
            ...
            print(
                f"{self.__class__.__name__}::ERROR::{self.name}::Provided pattern"
        ...
        * Determine the number of mesh vertices to be generated on edges, s.t. they match in the stitches
        ...
        if "stitches" in self.pattern:
            for stitch_id in range(len(self.pattern["stitches"])):
                ...
                # MYNOTE: right/wrong = visible/not visible side of the fabric
                stitch.swap = not (
                    len(stitch_spec) == 3 and "right_wrong" == stitch_spec[-1]
                )
                ...
                multi_edge = [
                    (p, e)
                    for (p, e) in [
                        (panel_name_0, edge_id0),
                        (panel_name_1, edge_id1),
                    ]
            ...
            print(
                f"{self.__class__.__name__}::INFO::No stitching information provided"
            )

    # !SECTION
    # SECTION -- generate per-panel meshes
    def _get_edge_in_verts(self, edge: Edge, plot=False):
        """
        ...
        if isinstance(edge.curve, svgpath.QuadraticBezier) or isinstance(
            ...
            curve_len = edge.curve.length()
            assert curve_len and isinstance(curve_len, float)

            curve_lengths = np.linspace(0, 1, n) * curve_len
        ...
        if isinstance(edge.curve, svgpath.Arc):
            ...
            points = edge.curve.points(ts)
            edge_in_vertices = [pat_utils.c_to_np(p) for p in points]
        # elif isinstance(edge.curve, svgpath.QuadraticBezier) or isinstance(edge.curve, svgpath.CubicBezier):
        #     pass
        # else:
        #     raise NotImplementedError(
        #         f'{self.__class__.__name__}::{self.name}::Unknown curve type {type(edge.curve)}')
    ...
    def _swap_stitch_ranges(self, stitch: Seam):
        ...
        * stitch_range_2 (list): Correctly ordered indices into stitch.edge_2 in stitch.panel_2
        """
        panel1, panel2 = (
            self.panels[stitch.panel_1],
            self.panels[stitch.panel_2],
        )
    ...
    def _stitch_two_diff_existent_glob_verts(
        self, glob1, glob2, glob_idx, stitch_id
    ):
        """
        ...
        * stitch_id (int): Stitch identifier indicating which stitch is currently performed
        ...
        panel_locids_above = np.array(self.verts_glob_loc, dtype=object)[
            repl_glob_ids
        ]
        ...
        self.stitch_segmentation[glob_min] += copy_stitch_ids + [
            "stitch_" + str(stitch_id)
        ]
    ...
    def _stitch_one_existent_glob_vert(
        ...
        * stitch_id (int): Stitch identifier indicating which stitch is currently performed
        ...
        self.verts_glob_loc[glob].append(
            (panel_name_not_glob, loc_id_not_glob)
        )
    ...
    def _stitch_none_existent_glob_verts(
        self, panel1, panel2, loc_id1, loc_id2, glob_idx, stitch_id
    ):
        """
    ...
    def _stitch_vertices(self):
        ...
        * same_panel_stitching_dict (dict): Dictionary storying the local vertex indices to which a local vertex
        ...
        # Store stichings of same panel (panelname,loc_id) -> loc_id
            ...
            for loc_id1, loc_id2 in zip(stitch_range_1, stitch_range_2):
                if (
                    ...
                    if (
                        stitch.panel_1,
                        loc_id1,
                    ) not in self.verts_loc_glob.keys():
                        self._stitch_same_loc_vertex(
                            panel1, loc_id1, glob_idx, stitch_id
                        )
                        glob_idx += 1
                    else:
                        glob_id = self.verts_loc_glob[
                            (stitch.panel_1, loc_id1)
                        ]
    ...
    def _check_same_panel_stitching(self, dic, global_ids):
        ...
        for g_id in global_ids:
            # All local verts corresponding to g_id
            ...
            for panel_name, loc_ids in l:
                if self.check_local_vertices_stitching(
                    dic, panel_name, loc_ids
                ):
                    return True
    ...
    def _valid_stitch_same_panel(
        self, stitch: Seam, same_panel_stitching_dict
    ):
        """
    ...
    def _is_stitching_valid(
        self, same_panel_stitching_dict, front_end_only=False
    ):
    ...
    def collapse_stitch_vertices(self):
        ...
        * self (BoxMesh object): Instance of BoxMesh class from which the function is called
        ...
        if not valid:
            print(
                f"{self.__class__.__name__}::{self.name}::ERROR::Invalid stitching. Unable to fix"
            )
    ...
    def _get_glob_ids(self, panel, face):
        ...
        for loc_id in face:
            if loc_id < n_stitches_panel:
                glob_indices.append(
                    self.verts_loc_glob[(panel.panel_name, loc_id)]
                )
            else:
                glob_indices.append(
                    loc_id + panel.glob_offset - n_stitches_panel
                )
    ...
    def _get_seam_gt_el(self, el_i, el_j, el_k, id1, id2, stitch_edges_gt):
        ...
        if low < min_el and min_el < up:
            ...
            print(
                f"{self.__class__.__name__}::WARNING::{self.name}::Impossible to set "
        ...
        for panelname in self.panelNames:
            ...
            for edge in panel.edges:
                if edge.label and edge.stitch_ref is None:
                    ...
                    # Exclude ends which are located elsewhere and may be in a stitch
                    e_verts = np.array(edge.vertex_range[1:-1])
                    labeled_verts = e_verts + len_B_verts - n_stitches_panel
                    self.vertex_labels.setdefault(edge.label, []).extend(
                        labeled_verts.tolist()
                    )
            ...
            for face in panel.panel_faces:
                ...
                if loc_stitch_ids:
                    self._store_to_orig_lens(
                        panel, face, f_glob_ids, stitch_edges_gt
                    )
    ...
    def save_box_mesh_obj(
        self, with_normals=False, in_uv_config={}, mat_name="panels_texture"
    ):
        """
        ...
        if not self.loaded:
            print(
                f"{self.__class__.__name__}::{self.name}::WARNING::Pattern is not yet loaded. Nothing saved"
            )
            return
    ...
    def serialize(
        ...
        self.save_box_mesh_obj(
            with_normals=with_v_norms, in_uv_config=uv_config
        )
        ...
        if self.paths.in_design_params.exists():
            ...
            print(
                f"{self.__class__.__name__}::{self.name}::WARNING::Path does not exist: {self.paths.in_design_params}"
            )
        if self.paths.in_body_mes.exists():
            shutil.copy(self.paths.in_body_mes, self.paths.body_mes)
        else:
            print(
                f"{self.__class__.__name__}::{self.name}::WARNING::Path does not exist: {self.paths.in_body_mes}"
            )
