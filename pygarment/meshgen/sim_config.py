...
class PathCofig:
    ...
    def _update_in_paths(self):
        ...
        if not self._samples_folder_name or self._use_default_body:
            ...
            self.bodies_path = (
                Path(self._system["body_samples_path"])
                / self._samples_folder_name
                / "meshes"
            )
        ...
        if (
            "body_sample" in body_dict["body"]
        ):  # Not present in default measurements
        ...
        self.body_seg = Path(self._system["bodies_default_path"]) / (
            ...
            if not self.use_smpl_seg
            else "smpl_vert_segmentation.json"
        )
    ...
    def _update_boxmesh_paths(self):
        ...
        self.g_box_mesh_compressed = (
            self.out_el / f"{self.boxmesh_tag}_boxmesh.ply"
        )
        self.g_mesh_segmentation = (
            self.out_el / f"{self.boxmesh_tag}_sim_segmentation.txt"
        )
        self.g_orig_edge_len = (
            self.out_el / f"{self.boxmesh_tag}_orig_lens.pickle"
        )
        self.g_vert_labels = (
            self.out_el / f"{self.boxmesh_tag}_vertex_labels.yaml"
        )
        self.g_texture_fabric = (
            self.out_el / f"{self.boxmesh_tag}_texture_fabric.png"
        )
...
class SimConfig:
    def __init__(self, sim_props):
        ...
        self.zero_gravity_steps = self.get_sim_props_value(
            sim_props, "zero_gravity_steps", 5
        )
        self.resolution_scale = self.get_sim_props_value(
            sim_props, "resolution_scale", 1.0
        )
        self.ground = self.get_sim_props_value(sim_props, "ground", True)

        # Stopping criteria
        self.static_threshold = self.get_sim_props_value(
            sim_props, "static_threshold", 0.01
        )
        self.max_sim_steps = self.get_sim_props_value(
            sim_props, "max_sim_steps", 1000
        )
        self.max_frame_time = int(
            self.get_sim_props_value(sim_props, "max_frame_time", 15)
        )
        self.max_sim_time = int(
            self.get_sim_props_value(sim_props, "max_sim_time", 25 * 60)
        )
        self.non_static_percent = self.get_sim_props_value(
            sim_props, "non_static_percent", 5
        )
        # Quality filter
        self.max_body_collisions = self.get_sim_props_value(
            sim_props, "max_body_collisions", 0
        )
        self.max_self_collisions = self.get_sim_props_value(
            sim_props, "max_self_collisions", 0
        )

        # Self-collision prevention properties
        self.enable_particle_particle_collisions = self.get_sim_props_value(
            sim_props_option, "enable_particle_particle_collisions", False
        )
        self.enable_triangle_particle_collisions = self.get_sim_props_value(
            sim_props_option, "enable_triangle_particle_collisions", False
        )
        self.enable_edge_edge_collisions = self.get_sim_props_value(
            sim_props_option, "enable_edge_edge_collisions", False
        )
        self.enable_body_collision_filters = self.get_sim_props_value(
            sim_props_option, "enable_body_collision_filters", False
        )

        # Attachment constraints
        self.enable_attachment_constraint = self.get_sim_props_value(
            sim_props_option, "enable_attachment_constraint", False
        )
        self.attachment_labels = self.get_sim_props_value(
            sim_props_option, "attachment_label_names", []
        )
        self.attachment_frames = self.get_sim_props_value(
            sim_props_option, "attachment_frames", 100
        )
        self.attachment_stiffness = self.get_sim_props_value(
            sim_props_option, "attachment_stiffness", []
        )
        self.attachment_damping = self.get_sim_props_value(
            sim_props_option, "attachment_damping", []
        )
        ...
        self.global_damping_factor = self.get_sim_props_value(
            sim_props_option, "global_damping_factor", 1.0
        )
        self.global_damping_effective_velocity = self.get_sim_props_value(
            sim_props_option, "global_damping_effective_velocity", 0.0
        )
        ...
        self.enable_global_collision_filter = self.get_sim_props_value(
            sim_props_option, "enable_global_collision_filter", False
        )
        self.enable_cloth_reference_drag = self.get_sim_props_value(
            sim_props_option, "enable_cloth_reference_drag", False
        )
        ...
        self.garment_edge_ke = self.get_sim_props_value(
            sim_props_material, "garment_edge_ke", 50000.0
        )  # default = 100.0
        self.garment_edge_kd = self.get_sim_props_value(
            sim_props_material, "garment_edge_kd", 10.0
        )  # default = 0.0

        # Area preservation
        self.garment_tri_ke = self.get_sim_props_value(
            sim_props_material, "garment_tri_ke", 10000.0
        )  # default = 100.0, small number = more elasticity
        self.garment_tri_kd = self.get_sim_props_value(
            sim_props_material, "garment_tri_kd", 1.0
        )  # default = 10.0
        self.garment_tri_ka = self.get_sim_props_value(
            sim_props_material, "garment_tri_ka", 10000.0
        )  # default = 100.0
        ...
        # Body material
        self.body_thickness = self.get_sim_props_value(
            sim_props_option, "body_collision_thickness", 0.0
        )
        self.body_friction = self.get_sim_props_value(
            sim_props_option, "body_friction", 0.5
        )
    ...
    def update_min_steps(self):
        ...
        if self.enable_body_smoothing:
            self.min_sim_steps = (
                self.smoothing_recover_start_frame + self.smoothing_num_steps
            )
        if self.enable_attachment_constraint:
            ...
            self.min_sim_steps = max(
                self.min_sim_steps, self.attachment_frames + 5
            )
