...
def get_command_args():
    ...
    parser.add_argument(
        "--replicate",
        "-re",
        help="Name of the dataset to re-generate. If set, other arguments are ignored",
        type=str,
        default=None,
    )
...
def body_sample(bodies: dict, path: Path, straight=True):
    ...
    mes_file = body_i["mes"]
    obj_file = (
        body_i["objs"]["straight"] if straight else body_i["objs"]["apart"]
    )
...
def has_pants(design: Dict) -> bool:
...
def assert_param_combinations(design, filter_belts=True):
    ...
    if (
        lower_name == "Skirt2"
        or lower_name == "GodetSkirt"
        and design["godet-skirt"]["base"]["v"] == "Skirt2"
        or lower_name == "SkirtLevels"
        and design["levels-skirt"]["base"]["v"] == "Skirt2"
    ):

        if design["skirt"]["ruffle"]["v"] > 1 and (
            not belt_name or design["waistband"]["waist"]["v"] > 1.0
        ):
            raise IncorrectElementConfiguration(
    ...
    if (
        lower_name in flare_skirts
        or lower_name == "SkirtLevels"
        and design["levels-skirt"]["base"]["v"] in flare_skirts
    ):
        # if Fitted belt of enough width not present -- check if "heavy"
        if (
            not belt_name
            or design["waistband"]["waist"]["v"] > 1.0
            or design["waistband"]["width"]["v"] <= 0.25
        ):
            length_param = design[
                (
                    "levels-skirt"
                    if lower_name == "SkirtLevels"
                    else "flare-skirt"
                )
            ]["length"]["v"]
...
# Generation loop
def generate(
    path, properties: Properties, sys_paths: Properties, verbose=False
):
    ...
    body_samples_path = (
        Path(sys_paths["body_samples_path"]) / properties["body_samples"]
    )
    ...
    default_body = BodyParameters(
        Path(sys_paths["bodies_default_path"])
        / (properties["body_default"] + ".yaml")
    )
    ...
    for i in range(properties["size"]):
        ...
        for _ in range(
            100
        ):  # Putting a limit on re-tries to avoid infinite loops
            ...
            try:
                if verbose:
                    ...
                    with open(
                        Path("./Logs") / f"{name}_design_params.yaml", "w"
                    ) as f:
                        yaml.dump(
                            {"design": new_design},
                            f,
                ...
                if (
                    piece_default.is_self_intersecting()
                    or piece_shaped.is_self_intersecting()
                ):
                    if verbose:
                ...
                if (default_sample_data / name).exists():
                    print(
                        "Generate::Info::Removed empty folder after unsuccessful sampling attempt",
                        default_sample_data / name,
                    )
                    shutil.rmtree(
                        default_sample_data / name, ignore_errors=True
                    )

                if (body_sample_data / name).exists():
                    print(
                        "Generate::Info::Removed empty folder after unsuccessful sampling attempt",
                        body_sample_data / name,
                    )
...
if __name__ == "__main__":
    ...
    if args.replicate is not None:
        ...
        props.set_basic(
            ...
            size=args.size,
            name=(
                f"{args.name}_{args.size}"
                if not args.batch_id
                else f"{args.name}_{args.size}_{args.batch_id}"
            ),
            to_subfolders=True,
        )
