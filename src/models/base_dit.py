...
class LabelEmbedder(nn.Module):
    """
    Embeds class labels into vector representations. Also handles label
...
class DiT(nn.Module):
    ...
    def forward(self, x, t, y):
        ...
        x: (N, C, H, W) tensor of spatial inputs
            (images or latent representations of images)
    ...
    def forward_with_cfg(self, x, t, y, cfg_scale):
        """
        Forward pass of DiT, but also batches the unconditional
        forward pass for classifier-free guidance.
        """
        # https://github.com/openai/glide-text2im/blob/main/notebooks/text2im.ipynb
        half = x[: len(x) // 2]
        combined = torch.cat([half, half], dim=0)
        model_out = self.forward(combined, t, y)
        # For exact reproducibility reasons, we apply
        # classifier-free guidance on only
        # three channels by default. The standard approach to cfg
        # applies it to all channels.
        # This can be done by uncommenting the following line and
        # commenting-out the line following that.
        # eps, rest = (
        #   model_out[:, :self.in_channels], model_out[:, self.in_channels:]
        #   )
