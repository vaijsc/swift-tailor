...
from assets.garment_programs.base_classes import (
    BaseBottoms,
...
class SkirtPanel(pyg.Panel):
    ...
    def __init__(
        ...
        x_shift_top = (
            low_width - top_width
        ) / 2  # to account for flare at the bottom

        # define edge loop
        self.right = (
            pyg.EdgeSeqFactory.side_with_cut(
                [0, 0], [x_shift_top, length], start_cut=bottom_cut / length
            )
            if bottom_cut
            else pyg.EdgeSequence(pyg.Edge([0, 0], [x_shift_top, length]))
        )
        self.waist = pyg.Edge(
        ...
        self.left = (
            pyg.EdgeSeqFactory.side_with_cut(
                self.waist.end, [low_width, 0], end_cut=bottom_cut / length
            )
            if bottom_cut
            else pyg.EdgeSequence(pyg.Edge(self.waist.end, [low_width, 0]))
        )
        ...
        self.interfaces = {
            ...
            "top": pyg.Interface(
                ...
                self.waist,
                ruffle=(
                    self.waist.length() / match_top_int_to
...
class ThinSkirtPanel(pyg.Panel):
    ...
    def __init__(
        self, name, top_width=10, bottom_width=20, length=70, b_curvature=0
    ) -> None:
        super().__init__(name)
...
class FittedSkirtPanel(pyg.Panel):
    ...
    def __init__(
        ...
        waist,
        hips,
        hips_depth,  # TODOLOW Half measurement instead of a quarter
        length,
        hipline_ext=1,
        dart_position=None,
        dart_frac=0.5,
        ...
        w_diff = (
            hips - waist
        )  # Assume its positive since waist is smaller then hips
        ...
        if pyg.utils.close_enough(flare, 1):  # skip optimization
            right_bottom = pyg.Edge(
                [hips - low_width, angle_shift], [0, length]
            )
        else:
        ...
        # top
        top = pyg.Edge(
            right[-1].end, [hips * 2 - hw_shift, length + adj_hips_depth]
        )
        ...
        if pyg.utils.close_enough(
            flare, 1
        ):  # skip optimization for straight skirt
        ...
        self.interfaces = {
            ...
            "right": pyg.Interface(
                self, right, [1] * (len(right) - 1) + [hipline_ext]
            ),
            "left": pyg.Interface(
                self, left, [hipline_ext] + [1] * (len(left) - 1)
            ),
        }
        ...
        if w_diff > hw_shift:
            ...
            self.interfaces["top"] = pyg.Interface(
                ...
                int_edges,
                ruffle=(
                    int_edges.length() / match_top_int_to
                    if match_top_int_to is not None
                    else 1.0
                ),
            )
    ...
    def add_darts(
        self, top, dart_width, dart_depth, dart_position, double_dart=False
    ):
        ...
        if double_dart:
            ...
            dart_shape_full = pyg.EdgeSeqFactory.dart_shape(
                dart_width / 2, dart_depth
            )
            dart_shape_small = pyg.EdgeSeqFactory.dart_shape(
                dart_width / 2, dart_depth * 0.9
            )
            darts = [
                dart_shape_small,
                dart_shape_full,
...
class PencilSkirt(StackableSkirtComponent):
    def __init__(
        self, body, design, tag="", length=None, rise=None, slit=True, **kwargs
    ) -> None:
        ...
        if design["style_side_cut"]["v"] is not None:
            ...
            style_shape_l, style_shape_r = shape_class(
                ...
                d_rays=depth * 0.2,
                filename=(
                    design["style_side_file"]["v"]
                    if "style_side_file" in design
                    else None
                ),
            )
        else:
        ...
        if length is None:
            length = (
                design["length"]["v"] * body["_leg_length"]
...
class Skirt2(StackableSkirtComponent):
    ...
    def __init__(
        ...
        if length is None:
            length = (
                hip_line + design["length"]["v"] * body["_leg_length"]
            )  # Depends on leg length
        ...
        self.front = SkirtPanel(
            ...
            length=length,
            ruffles=(
                design["ruffle"]["v"] if top_ruffles else 1
            ),  # Only if on waistband
            flare=design["flare"]["v"],
            bottom_cut=(
                design["bottom_cut"]["v"] * design["length"]["v"]
                if slit
                else 0
            ),
            match_top_int_to=(body["waist"] - body["waist_back_width"]),
...
class SkirtManyPanels(BaseBottoms):
    ...
    def __init__(self, body, design, tag="", rise=None, min_len=5) -> None:
        tag_extra = str(
            design["flare-skirt"]["skirt-many-panels"]["n_panels"]["v"]
        )
        ...
        dist = self.front.interfaces["bottom"].edges.length() / (
            2 * np.tan(np.pi / n_panels)
        )
        ...
        # Create new panels
        self.subs = pyg.ops.distribute_Y(
            self.front, n_panels, name_tag="skirt_panel"
        )
        ...
        self.interfaces = {
            "top": pyg.Interface.from_multiple(
                *[sub.interfaces["top"] for sub in self.subs]
            )
        }
