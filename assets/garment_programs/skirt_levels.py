...
class SkirtLevels(BaseBottoms):
    ...
    def __init__(self, body, design, rise=None) -> None:
        ...
        self.subs.append(
            base_skirt_class(
                body, design, length=self.base_len, rise=self.rise, slit=False
            )
        )

        if (
            hasattr(base := self.subs[0], "design")
        ...
        for i in range(n_levels):
            ...
            lbody["waist"] = (
                ruffle * self.subs[-1].interfaces["bottom"].edges.length()
            )
            lbody["waist_back_width"] = (
                ruffle * self.subs[-1].interfaces["bottom_b"].edges.length()
            )
            self.subs.append(
                level_skirt_class(
                    lbody,
            ...
            self.subs[-1].rotate_by(
                R.from_euler("XYZ", [0, 0, -self.angle], degrees=True)
            )
        ...
        self.interfaces = {"top": self.subs[0].interfaces["top"]}
    ...
    def eval_length(self, ldesign, body):
        ...
        self.level_len = (total_length - self.base_len) / ldesign[
            "num_levels"
        ]["v"]

        # Add hip_line (== zero length)
        self.base_len = (
            body["hips_line"] * ldesign["rise"]["v"] + self.base_len
        )
