    ...
    def __init__(
        ...
        self.edges.append(
            pyg.CircleEdgeFactory.from_points_radius(
                ...
                [dist_w / 2, 0],
                radius=top_rad,
                large_arc=halfarc > np.pi / 2,
            )
        )

        self.edges.append(
            pyg.Edge(self.edges[-1].end, [dist_out / 2, -vert_len])
        )
        ...
        self.interfaces = {
            "top": pyg.Interface(
                ...
                self.edges[0],
                ruffle=(
                    self.edges[0].length() / match_top_int_proportion
            ...
            "bottom": pyg.Interface(
                ...
                self.edges[2],
                ruffle=(
                    self.edges[2].length() / match_bottom_int_proportion
...
class SkirtCircle(StackableSkirtComponent):
    ...
    def add_cut(self, panel, design, sk_length):
        ...
        width, depth = (
            design["cut"]["width"]["v"] * sk_length,
            design["cut"]["depth"]["v"] * sk_length,
        )
