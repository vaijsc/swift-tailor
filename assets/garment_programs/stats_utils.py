...
def sleeve_length(design):
    # Sleeve length
    sleeve_len_r = (
        design["sleeve"]["length"]["v"]
        if not design["sleeve"]["sleeveless"]["v"]
        else 0
    )
    sleeve_len_l = (
        design["left"]["sleeve"]["length"]["v"]
        if design["left"]["enable_asym"]["v"]
        and not design["left"]["sleeve"]["sleeveless"]["v"]
        else 0
    )
...
def vertical_len(design):
    ...
    wb_len = (
        design["waistband"]["width"]["v"] if design["meta"]["wb"]["v"] else 0
    )
