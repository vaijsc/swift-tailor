...
class Insert(pyg.Panel):
    def __init__(self, id, width=30, depth=30) -> None:
        ...
        self.edges = pyg.EdgeSeqFactory.from_verts(
            [0, 0], [width / 2, depth], [width, 0], loop=True
        )

        self.interfaces = [pyg.Interface(self, self.edges[:2])]
...
class GodetSkirt(BaseBottoms):
    ...
    def __init__(self, body, design, rise=None) -> None:
        ...
        self.interfaces = {"top": self.base.interfaces["top"]}

    def inserts(
        self, bottom_edge, panel, ins_w, ins_depth, num_inserts=3, cuts_dist=0
    ):
        ...
        if cut_width < 1:
            ...
            if self.verbose:
                print(
                    f"{self.__class__.__name__}::WARNING:: Cannot place {num_inserts} cuts "
        ...
        insert = Insert(0, width=ins_w, depth=ins_depth).translate_by(
            [
                x_shift - num_inserts * ins_w / 2 + ins_w / 2,
                y_base + ins_depth,
                z_transl,
            ]
        )
        ...
        # should be the same on the skirt and the insert
        ...
        if side_len > cut_width / 2:  # Normal case
            ...
            if self.verbose:
                print(
                    f"{self.__class__.__name__}::WARNING::Requested cut_width ({old_cut_width:.2f}) "
        ...
        right = (
            z_transl < 0
        )  # NOTE: heuristic corresponding to skirts in our collection

        for i in range(num_inserts):
            # start_offest + i * stride
            offset = cut_width / 2 + (cuts_dist / 2 if i == 0 else cuts_dist)
            ...
            # New edge that needs to be cutted -- on the next step
            bottom_edge = new_bottom[-1]
            ...
            self.stitching_rules.append(
                (
                    self.subs[
                        -1 - i if right else -(num_inserts - i)
                    ].interfaces[0],
                    cut_interface,
                )
            )
