...
class PantsHalf(BaseBottoms):
    def __init__(self, tag, body, design, rise=None) -> None:
        ...
        length, cuff_len = (
            design["length"]["v"],
            design["cuff"]["cuff_len"]["v"],
        )
        if design["cuff"]["type"]["v"]:
            if (
                length - cuff_len < design["length"]["range"][0]
            ):  # Min length from paramss
        ...
        self.stitching_rules = pyg.Stitches(
            (
                self.front.interfaces["outside"],
                self.back.interfaces["outside"],
            ),
        ...
        if design["cuff"]["type"]["v"]:
            ...
            cdesign["cuff"]["b_width"]["v"] = (
                pant_bottom.edges.length() / design["cuff"]["top_ruffle"]["v"]
            )
            ...
            self.stitching_rules.append(
                (pant_bottom, self.cuff.interfaces["top"])
            )
...
class Pants(BaseBottoms):
    def __init__(self, body, design, rise=None) -> None:
        ...
        self.stitching_rules = pyg.Stitches(
            (
                self.right.interfaces["crotch_f"],
                self.left.interfaces["crotch_f"],
            ),
            (
                self.right.interfaces["crotch_b"],
                self.left.interfaces["crotch_b"],
            ),
        )
