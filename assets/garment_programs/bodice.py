...
from assets.garment_programs import collars, sleeves, tee
...
class BodiceFrontHalf(BaseBodicePanel):
    def __init__(self, name, body, design) -> None:
        ...
        back_adjustment = sh_tan * (
            body["back_width"] / 2 - body["shoulder_w"] / 2
        )
        ...
        side_d_depth = 0.75 * (self.width - bust_point)
        ...
        self.interfaces = {
            ...
            "outside": pyg.Interface(self, side_interface),
...
class BodiceBackHalf(BaseBodicePanel):
    ...
    def __init__(self, name, body, design) -> None:
        ...
        if waist < self.get_width(
            self.edges[2].end[1] - self.edges[2].start[1]
        ):
            w_diff = waist_width - waist
            # NOTE: don't take from sides if the difference is too small
            side_adj = 0 if w_diff < 4 else w_diff / 6
...
class BodiceHalf(pyg.Component):
    ...
    def __init__(self, name, body, design, fitted=True) -> None:
        ...
        if design["shirt"]["strapless"]["v"] and fitted:
    ...
    def make_strapless(self, body, design):
        ...
        self._adjust_top_level(
            ...
            out_depth,
            f_in_depth,
            target_remove=(len_front - len_back),
        )
        ...
        # Add a label
        self.ftorso.interfaces["shoulder"].edges.propagate_label(
            "strapless_top"
        )
        self.btorso.interfaces["shoulder"].edges.propagate_label(
            "strapless_top"
        )

    def _adjust_top_level(
        self, panel, out_level, in_level, target_remove=None
    ):
...
class Shirt(pyg.Component):
    ...
    def eval_dep_params(self, design):
        ...
        if design["left"]["enable_asym"]["v"]:
            ...
            design["left"]["shirt"]["length"]["v"] = design["shirt"]["length"][
                "v"
            ]

            design["left"]["collar"].update(fc_depth={}, bc_depth={})
            design["left"]["collar"]["fc_depth"]["v"] = design["collar"][
                "fc_depth"
            ]["v"]
            design["left"]["collar"]["bc_depth"]["v"] = design["collar"][
                "bc_depth"
            ]["v"]
