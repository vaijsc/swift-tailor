...
vomg, rdimg, _ = omgutil.preview_omg(
    omage,
    decoder_kwargs=dict(
        render_mode="segcolor",
        save_path=f"{cwd}/temp/omage_preview_B0742FHDJF.blend",
    ),
)
...
vomg, rdimg, _ = omgutil.preview_omg(
    omage,
    geometry_only=True,
    render_kwargs=dict(camera_position=(2.0, -2.0, 1), shadow_catcher=True),
    decoder_kwargs=dict(render_mode="segcolor", save_path=None),
)
visutil.showImg(vomg)
