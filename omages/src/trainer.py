...
from xgutils import datautil, optutil, qdaq, sysutil
...
class Trainer:
    def default_opt(self):
        return dict(
            ...
            gpus=[
                0
            ],  # int for number, -1/'auto' for all, list for specified ones
            max_epochs=100,
            min_epochs=1,
            max_time=None,
            copy_ckpt_from="",
    ...
    def __init__(self, opt, root_dir=DEFAULT_ROOT, mode="test", gpus=[0]):
        ...
        self.project_name = (
            self.opt.project_name
            if hasattr(self.opt, "project_name")
            else "project"
        )
        ...
        # check more details here: https://github.com/Lightning-AI/lightning/issues/12997
        if trainer_opt["use_tensorcores"]:
            # enable tensorcores by not choosing "highest" precision
        ...
        self.MainTrainerOptions = dict(
            ...
            check_val_every_n_epoch=self.trainer_opt[
    ...
    def get_logger(self):
        ...
        if trainer_opt["logger"] == "tensorboard":
            ...
            version = 0
            if (
                "mode" in trainer_opt["logger_kwargs"]
                and trainer_opt["logger_kwargs"]["mode"] == "disabled"
            ):
            ...
            logger = loggers.WandbLogger(
                name=self.opt.expr_name,
                project=self.project_name,
                save_dir=self.minfo["logs_dir"],
                dir=self.minfo["logs_dir"],
                version=version,
    ...
    def train(self, resume_from=None):
        ...
        self.trainer.fit(
            self.model, datamodule=self.data_module, ckpt_path=resume_from
        )
        print(
            "Model trained, best model path: ",
            self.checkpoint_callback.best_model_path,
        )
        if (
            self.checkpoint_callback.best_model_path is not None
            and self.checkpoint_callback.best_model_path != ""
        ):
...
class ExpJob(qdaq.Job):
    ...
    def run(self, cuda_device_id):
        ...
        sys.stdout = open(
            os.path.join(self.minfo["logs_dir"], "stdout.out"), "w"
        )
        trainer.train()
        return True
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--opts", type=str, nargs="+", help="path to project options"
    )
