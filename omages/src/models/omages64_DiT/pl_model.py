...
class Model(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        bDiT = self.model = self.baseDiT = sysutil.instantiate_from_opt(
            self.baseDiT_cfg
        )
        ...
        if init_from_ckpt is not None:
            ...
            self.load_state_dict(
                torch.load(init_from_ckpt, map_location=self.device)[
                    "state_dict"
                ],
                strict=False,
            )
    ...
    def forward_with_cfg(self, x, t, cx, y, cfg_scale):
        return self.baseDiT.forward_with_cfg(
            x, cx=cx, y=y, t=t, cfg_scale=cfg_scale
        )
    ...
    def training_step(self, batch, batch_idx, stage="train"):
        ...
        loss_dict = self.diffusion.training_losses(
            self, x_start=clean_images, t=t, model_kwargs=model_kwargs
        )
    ...
    def sample(self, batch, infer_steps="250", fp16=False, **kwargs: dict):
        ...
        z = torch.randn(
            n,
            self.baseDiT.in_channels,
            self.baseDiT.input_size,
            self.baseDiT.input_size,
            device=self.device,
            dtype=self.dtype,
        )
        ...
        cx = (
            torch.cat([cond_images, cond_images * 0 - 1.0])
            if cond_images is not None
            else None
        )
        model_kwargs = dict(y=y, cx=cx, cfg_scale=4.0)
