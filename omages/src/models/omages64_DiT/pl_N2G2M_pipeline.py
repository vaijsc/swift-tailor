...
from xgutils import omgutil, optutil, plutil, ptutil, sysutil
...
class Model(LightningModule):
    def __init__(self, **kwargs):
        ...
        # dummy parameter to avoid 'RuntimeError: DistributedDataParallel is not needed when a module doesn't have any parameter that requires a gradient' error.
    ...
    def configure_optimizers(self):
        optim = torch.optim.AdamW(self.parameters(), lr=1e-4, weight_decay=0)
...
def cfg2model(path, ckpt_path):
    ...
    model.load_state_dict(
        torch.load(ckpt_path, map_location=model.device)["state_dict"]
    )
    return model
...
class N2G2MCallback(plutil.ComputeGraphCallback):
    def __init__(
        ...
        G2M_cfg,
        G2M_ckpt,
        if_load_saved_compute=[True, True, False],
        mode="",
        infer_steps="250",
        **kwargs,
    ):
    ...
    def N2G_compute(self, data_dict, input_name=""):
        ...
        sampled_output = N2G_module.sample(
    ...
    def G2M_compute(self, data_dict, input_name=""):
        ...
        sampled_output = G2M_module.sample(
            batch=batch, infer_steps=self.infer_steps
        )
        computed = dict(batch=batch, sampled_output=sampled_output)
        return computed
...
def visualize(computed, input_name="", mode=""):
    ...
    if "G2T" in mode or "G2M" in mode:
        geo = (
            computed["batch"]["cond_img"].transpose(0, 2, 3, 1)[0] * 0.5 + 0.5
        )
        omg = np.concatenate(
