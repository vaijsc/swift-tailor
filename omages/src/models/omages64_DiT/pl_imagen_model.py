...
# import ema_pytorch
import torch_ema
from imagen_pytorch.imagen_pytorch import Imagen, NullUnet, Unet
from lightning.pytorch import LightningModule, Trainer, callbacks
from omegaconf import DictConfig, OmegaConf
from xgutils import omgutil, plutil, ptutil, sysutil
...
class Model(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.unets = unet = Unet(  # follows original paper
            ...
            # try to solve color shifting problem. https://github.com/lucidrains/imagen-pytorch/issues/72#issuecomment-1272591508
        ...
        if init_from_ckpt is not None:
            ...
            self.load_state_dict(
                torch.load(init_from_ckpt)["state_dict"], strict=False
            )
    ...
    def training_step(self, batch, batch_idx, stage="train"):
        ...
        # material (8 channels) RGB, Normal xyz, Metallic, Roughness
        cond_images = batch.get("cond_img", None)

        if cond_images is not None:
            assert (
                cond_images.shape[1] == self.cond_omage_channels
            ), f"cond_images.shape[1] {cond_images.shape[1]} != self.cond_omage_channels {self.cond_omage_channels}"
        ...
        self.log_dict({log_key: loss}, prog_bar=True, sync_dist=True)
    ...
    def sample(self, batch, infer_steps="250", fp16=False, **kwargs: dict):
        cond_images = (
            batch["cond_img"].to(device=self.device, dtype=self.dtype)
            if batch.get("cond_img", None) is not None
            else None
        )
...
class Visualizer(plutil.VisCallback):
    ...
    def compute_batch(self, batch, input_name=""):
        sampled_output = self.pl_module.sample(
            batch=batch, infer_steps=self.infer_steps
        )
    ...
    def visualize_batch(self, computed, input_name=""):
        ...
        sampled_output = np.clip(sampled_output * 0.5 + 0.5, 0.0, 1.0)
        gt_img = computed["batch"]["img"].transpose(0, 2, 3, 1)
        gt_img = np.clip(gt_img * 0.5 + 0.5, 0.0, 1.0)
        gt_omg = computed["batch"]["omage"].transpose(0, 2, 3, 1)
        gt_omg = np.clip(gt_omg * 0.5 + 0.5, 0.0, 1.0)
        imgs = dict()
        ...
        if "vGeo" in self.mode:
            omg = gt_img[0]
            vomg, _, _ = omgutil.preview_omg(
                omg,
                return_rendered=True,
                geometry_only=True,
                combine_vomgs=True,
                decoder_kwargs=dict(
                    render_mode="segcolor",
                    preset_blend="xgutils/assets/preset_glb.blend",
                ),
            )
        ...
        if "G2T" in self.mode or "G2M" in self.mode:
            geo = (
                computed["batch"]["cond_img"].transpose(0, 2, 3, 1)[0] * 0.5
                + 0.5
            )
            omg = np.concatenate(
                [geo, np.zeros((geo.shape[0], geo.shape[1], 8))], axis=-1
            )
