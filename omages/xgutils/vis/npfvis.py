...
def plot_2d_sample(
    ...
    if ax is None:
        ...
        fig = ax.figure
    v, e = geoutil.array2mesh(Ytg, dim=2, coords=Xtg, thresh=0.5)
    if v is None:
    ...
    if len(v) > 0 and len(e) > 0:
        num, label = scipy.sparse.csgraph.connected_components(
            scipy.sparse.coo_matrix(
                (np.ones(e.shape[0], dtype=int), e.T), shape=(v.shape[0],) * 2
            ).tocsr(),
            directed=False,
        )
...
def plot_2d_samples(
    ...
    for i in range(len(ys)):
        fig, ax = plot_2d_sample(
            Xtg, ys[i][0], style=ys[i][1], zorder=i, ax=ax
        )
...
def plot_3d_recon(
    ...
    vert, face = geoutil.array2mesh(
        Ytg, dim=3, coords=Xtg, thresh=0.5, if_decimate=if_decimate
    )
    img = fresnelvis.renderMeshCloud(
        mesh={"vert": vert, "face": face}, meshC=meshC, **camera_kwargs
    )
...
def plot_3d_sample(
    ...
    Xtg,
    Ytg: np.ndarray = None,
    pred_y: np.ndarray = None,
    cloudR=0.01,
    camera_kwargs=dict(),
    show_images=["GT", "pred", "context"],
):
    context_x, context_y, target_x, target_y, pred_target_y = (
        Xct,
        Yct,
        Xtg,
        Ytg,
    ...
    if target_y is not None:
        vert, face = geoutil.array2mesh(
            Ytg, dim=3, coords=Xtg, thresh=0.5, if_decimate=True
        )
    ...
    if pred_target_y is not None:
        ...
        if context_x.shape[0] > 0:
            ...
            cloud = None
            cerror_scaled = None
        meshcloud = fresnelvis.renderMeshCloud(
            mesh=shape,
            cloud=cloud,
            cloudC=cerror_scaled,
            cloudR=cloudR,
            **camera_kwargs
        )
    ...
    if context_x is not None:
        cloud = fresnelvis.renderMeshCloud(
            mesh=None,
            cloud=context_x,
            cloudC=cerror_scaled,
            cloudR=cloudR,
...
def plot_3d_samples(
    ...
    context_x, context_y, target_x, target_y, pred_target_ys = (
        Xct,
        Yct,
        Xtg,
        Ytg,
    ...
    datas = []
    _, (GT_img, predmean_img, contextmean_img) = plot_3d_sample(
        Xct,
        Yct,
        Xtg,
        Ytg,
    ...
    for i in range(samples):
        ...
        _, (pred_img, context_img) = plot_3d_sample(
            Xct,
            Yct,
            Xtg,
            Ytg,
