...
def vis_VoxelXRay(
    voxel, axis=0, duration=10.0, target_path="/studio/temp/xray.mp4"
):
    ...
    for i in sysutil.progbar(range(voxel.shape[axis])):
        nvox = voxel.copy()
        nvox[i:, :, :] = 0
        voxv, voxf = geoutil.array2mesh(
            nvox.reshape(-1),
            thresh=0.5,
            coords=nputil.makeGrid(
                [-1, -1, -1], [1, 1, 1], [64, 64, 64], indexing="ij"
            ),
        )
        # voxv[]
        ...
        img = fresnelvis.renderMeshCloud(
            {"vert": voxv, "face": voxf}, **dflt_camera, axes=True
        )
        imgs.append(img)
...
def OctreePlot3D(tree, dim, depth, **kwargs):
    ...
    maxdep = tdepth.max()
    renderer = fresnelvis.FresnelRenderer(
        camera_kwargs=dict(
            camPos=np.array([1.5, 2, 2]), resolution=(1024, 1024)
        )
    )  # .add_mesh({"vert":vert, "face":face})
    for i in range(len(tdepth)):
        ...
        renderer.add_bbox(
            bb_min=bb_min,
            bb_max=bb_max,
            color=plt.cm.plasma(dep / maxdep)[:3],
            radius=0.001 * dep**1.5,
            solid=0.0,
        )
    img = renderer.render()
    return img
...
def SparseVoxelPlot(
    ...
    voxel_inds = ptutil.unravel_index(
        torch.from_numpy(sparse_voxel), shape=(2**depth,) * 3
    )
    voxel_coords = ptutil.ths2nps(
        ptutil.index2point(voxel_inds, grid_dim=grid_dim)
    )
...
def IndexVoxelPlot(
    ...
    **kwargs
    ...
    voxel_inds = ptutil.unravel_index(
        torch.from_numpy(pos_ind), shape=(2**depth,) * 3
    )
    voxel_coords = ptutil.ths2nps(
        ptutil.index2point(voxel_inds, grid_dim=grid_dim)
    )
...
def meshCloudPlot(Ytg, Xbd):
    ...
    img = fresnelvis.renderMeshCloud(
        {"vert": vert, "face": face}, cloud=Xbd, cloudR=0.01, **vis_camera
    )
    return img
...
def plot_volume(volume, show=True, contour_mode=True, slice_dim=1, zoom=3):
    ...
    if slice_dim == 0:
        ...
        pixels[: fvtrans.shape[0]] = (
            fvtrans  # [X Y Z] -> [ (X Y Z) ] -> [ X*Y*Z+padd ]
        )
        rear = einops.rearrange(
            pixels,
            "(n m y z) -> (n y) (m z)",
            y=cdim,
            z=cdim,
    ...
    if zoom > 1:
        ...
        rear = torch.nn.functional.interpolate(
            rear, scale_factor=zoom, mode="bilinear", align_corners=True
        )
        rear = einops.rearrange(
            rear, "(n m) 1 x y -> (n x) (m y)", n=vis_dim, m=vis_dim
        ).numpy()

        # scipy.ndimage.zoom(rear, zoom_factor, order=1)

    if show == True:
        fig, ax = visutil.SDF2DPlot(
            gridData=rear,
            im_origin="upper",
...
def render_polyloop_explosion(
    polyloops,
    radius=0.01,
    loop_vid=False,
    extent=[0.0, 0.2],
    frames=16,
    camera_kwargs=fresnelvis.dflt_camera,
    show_face=False,
    ...
    **kwargs
    ...
    camera_trajectory = parse_camera_trajectory(
        camera_trajectory, frames=frames, camera_kwargs=camera_kwargs
    )
    ...
    for ei, exi in sysutil.progbar(enumerate(ext)):
        ...
        img = fresnelvis.render_polyloop(
            v,
            f,
            modes=render_mode,
            camera_kwargs=camera_kwargs,
...
def generate_lemniscate_trajectory(
    camera_kwargs=fresnelvis.dfltw_camera, scale=1.0, frames=32
):
    trans, rot = fresnelvis.get_world2cam(
        camera_kwargs["camPos"],
        camera_kwargs["camLookat"],
        camera_kwargs["camUp"],
    )
...
def parse_camera_trajectory(
    camera_trajectory, frames=32, camera_kwargs=fresnelvis.dfltw_camera
):
    ...
    if type(camera_trajectory) is str:
        if camera_trajectory == "lemniscate":
            camera_trajectory = generate_lemniscate_trajectory(
                scale=1, frames=frames, camera_kwargs=ckwargs
            )
        elif camera_trajectory == "circular":
            camera_trajectory = generate_circular_trajectory(
                scale=1, frames=frames, camera_kwargs=ckwargs
            )
        elif camera_trajectory == "round":
            camera_trajectory = generate_round_trajectory(
                scale=1, frames=frames, camera_kwargs=ckwargs
            )
