...
def mesh_color_by_normal(vert, face):
    ...
    N = np.divide(
        N,
        np.linalg.norm(N, axis=1).reshape(-1, 1),
        out=np.zeros_like(N),
        where=np.linalg.norm(N, axis=1).reshape(-1, 1) != 0,
    )
    C = (N + 1) / 2
...
def add_axes(ax, bmin, bmax):
    ...
    plt.plot(
        [bmin, bmax], [bmax, bmax], [bmin, bmin], linewidth=1.5, color="red"
    )

    plt.plot(
        [bmin, bmin], [bmax, bmin], [bmin, bmin], linewidth=1.5, color="green"
    )

    plt.plot(
        [bmin, bmin], [bmax, bmax], [bmin, bmax], linewidth=1.5, color="blue"
    )
...
def add_mesh(
    ...
    collection = Poly3DCollection(
        T, closed=True, linewidth=0.05, facecolor=C, edgecolor="black"
    )
