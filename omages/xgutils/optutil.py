...
def get_opt(yaml, root_dir=DEFAULT_ROOT, src_name="src"):
    ...
    opt["meta_info"] = generate_meta_info(
        root_dir=root_dir, name=name, src_name=src_name
    )
    return opt
...
def expr_mkdirs(opt):
    m = opt["meta_info"]
    sysutil.mkdirs(
        [m["expr_dir"], m["logs_dir"], m["checkpoints_dir"], m["results_dir"]]
    )
...
def load_option(path, default_path=None):
    ...
    if inherit_from is not None:
        full_path = os.path.abspath(
            os.path.join(os.path.dirname(path), inherit_from)
        )
