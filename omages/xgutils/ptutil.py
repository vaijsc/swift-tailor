...
def print_cuda_stats():
    stats = "Current CUDA usage: %.2f GB, Max usage %.3f GB" % (
...
def array2NDCube(tensor, N=3):
    vox_dim = np.ceil(tensor.shape[0] ** (1.0 / N)).astype(int)
...
def np2th(array, device="cuda"):
    ...
    if (
        type(tensor) is np.ndarray
        and np.issubdtype(tensor.dtype, np.str_) == False
    ):
...
def nps2ths(arrays, device="cuda"):
    if type(arrays) is dict:
        ...
        for key in arrays:
            if (
                type(arrays[key]) is dict
                or type(arrays[key]) is list
                or type(arrays[key]) is tuple
            ):
...
def th2device(tensor, device="cpu"):  # ['cpu', 'cuda']
    if type(tensor) is torch.Tensor:
        ...
        type(tensor), torch.distributions.distribution.Distribution
    ):
...
def ths2device(tensors, device="cpu"):
    if type(tensors) is dict:
        ...
        for key in tensors:
            ...
            if (
                type(tensor) is dict
                or type(tensor) is list
                or type(tensor) is tuple
            ):
...
def unfold_cube(tensor, last_dims=2, size=2, step=2, flatten=True):
    ...
    if flatten == True:
        ...
        unfolded = unfolded.reshape(
            *(unfolded.shape[: -2 * last_dims]),
            total_size // unfold_size,
            unfold_size,
        )
    return unfolded
...
def fold_cube(unfolded, N=3):
    ...
    batch_dims = len(batch_shape)
    vox_dim = np.ceil(np.power(unfolded.shape[-1], 1.0 / N)).astype(int)
    unfolded = unfolded.reshape(*batch_shape, *((vox_dim,) * (2 * N)))
    folded = unfolded
    for i in range(N):
        folded = torch.cat(
            torch.split(folded, 1, dim=batch_dims + i), dim=batch_dims + N + i
        )
    for i in range(N):
...
def compress_voxels(voxel, packbits=True):
    ...
    divided = unfold_cube(
        torch.from_numpy(voxel), last_dims=3, size=16, step=16
    ).numpy()
    empty = divided.sum(axis=-1) == 0
    full = divided.sum(axis=-1) == 16**3
    partial = np.logical_and(1 - full, 1 - empty)
    empty_idx, full_idx, partial_idx = (
        np.where(empty)[0],
        np.where(full)[0],
        np.where(partial)[0],
    )
    shape_vocab = np.zeros(
        (1 + 1 + len(partial_idx), 16 * 16 * 16), dtype=np.bool
    )
    ...
    assert (
        shape_vocab[vocab_idx] != divided
    ).sum() == 0, "Invalid compression"
    if packbits == True:
...
def einops_compress_voxels(grid, packbits=False):
    # assert(voxel.shape[-1]==256), "Only 256-> 16x16 dims is supported"
    assert (
        grid.shape[-1] % block_size == 0
    ), "Irregular grid (can not devide block_size)"
    block_num = grid.shape[-1] // block_size
    blocked = rearrange(
        grid,
        "(x b1) (y b2) (z b3) -> x y z (b1 b2 b3)",
        b1=block_size,
        b2=block_size,
        b3=block_size,
    )
    ...
    shape_vocab = np.zeros(
        (1 + 1 + len(partial_idx), block_size**3), dtype=np.float32
    )
    ...
    # shape_vocab = shape_vocab.astype(bool)
    assert (
        shape_vocab[vocab_idx] != blocked
    ).any() == 0, "Invalid compression"
    # if packbits==True:
    #    shape_vocab = np.packbits(shape_vocab, axis=-1)
    return shape_vocab, vocab_idx  # uint8, int16
...
def fold_unittest():
    ...
    voxels = np.random.rand(256, 256, 256) > 0.5
...
def ravel_unittest():
    ...
    assert (npunravel == (unraveled[0].numpy())).all(), print(
        npunravel, "\n", unraveled[0].numpy()
    )
    raveled = ravel_index(unraveled, (3, 3))
    assert (idx == (raveled[0].numpy())).all(), print(
        idx, "\n", raveled[0].numpy()
    )
...
def point2index(
    points, grid_dim=32, ravel=False, ravel_type="flat", ret_relative=False
):
    ...
    float_index = torch.clamp(
        torch.round(shift_points), min=0.0, max=grid_dim - 1
    )
    ...
    if ravel == True:
        if ravel_type == "flat":
            ...
            assert np.mod(depth, 1) == 0.0, np.mod(depth, 1)
...
def point2tree(points, depth=6, max_length=-1):
    ...
    # timer.update()
    zorder = point2index(
        points, grid_dim=grid_dim, ravel=True, ravel_type="zorder"
    )
    # timer.update()
...
def point2voxel(points, grid_dim=32, ret_coords=False):
    ...
    voxel = torch.zeros(
        points.shape[0], *((grid_dim,) * points.shape[-1])
    ).type_as(points)
    ...
    binds = torch.repeat_interleave(
        torch.arange(points.shape[0]).type_as(points).long(), points.shape[1]
    )
    if points.shape[-1] == 2:
...
def subbatch_run_model(
    model, batch_input, other_input=dict(), sub_batch_size=32, verbose=False
):
