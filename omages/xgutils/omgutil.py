...
from xgutils.miscutil import preset_blend, preset_glb
...
def tensor2omg(objamage_tensor, seg_color_palette=None):
    if objamage_tensor.dtype == np.uint16:
        objamage_tensor = objamage_tensor.astype(np.float64) / 65535.0
    ...
    if objamage_tensor.shape[-1] == 12:
        ...
        objnormal_map = np.zeros(
            (objamage_tensor.shape[0], objamage_tensor.shape[1], 3)
        )
        color_map = np.zeros(
            (objamage_tensor.shape[0], objamage_tensor.shape[1], 3)
        )
        metal_map = np.zeros(
            (objamage_tensor.shape[0], objamage_tensor.shape[1], 1)
        )
        rough_map = np.zeros(
            (objamage_tensor.shape[0], objamage_tensor.shape[1], 1)
        )

    lb = skimage.measure.label(occupancy_map[..., 0] > 0.5)
...
def load_omg(
    ...
    if omage.dtype == np.uint16:
        omage = omage.astype(np.float64) / 65535.0
        # clip to 0, 1
        omage = np.clip(omage, 0.0, 1.0)
    if resize is not None:
        ...
        if omage.shape[0] != resize[0] or omage.shape[1] != resize[1]:
            if advanced_downsample == False:
                # important to use order 0 and disable anti_aliasing
                omage = skimage.transform.resize(
                    omage,
                    resize,
                    order=resize_order,
                    preserve_range=True,
                ...
                omage = downsample_omg(
                    omage, factor=omage.shape[0] // resize[0]
                )["omg_down_star"]
    if trim_omg == True:
...
def meshing_objamage(objamage, prune_verts=True):
    ...
    occ = objamage["occupancy"] > 0.5
    vert = objamage["position"] * 2 - 1
    objnormal = (
        objamage["objnormal"] * 2 - 1
        if objamage.get("objnormal", None) is not None
        else None
    )
    ...
    if objnormal is not None:
        face_normal = np.cross(
            vert[face[:, 1]] - vert[face[:, 0]],
            vert[face[:, 2]] - vert[face[:, 0]],
        )
        flip_mask = (
            np.einsum("ij,ij->i", face_normal, objnormal[face[:, 0]]) < 0
        )
        face[flip_mask] = face[flip_mask][:, [0, 2, 1]]

    uv = nputil.makeGrid(
        [0, 0], [1, 1], shape=(occ.shape[0], occ.shape[1]), mode="on"
    )
    # swap x, y to match the image coordinate system
    uv[..., [0, 1]] = uv[..., [1, 0]]
    meshing_ret = dict(vert=vert, face=face, uv=uv)
    for key in objamage:
        if key not in ["vert", "face", "uv"] and objamage[key] is not None:
            meshing_ret[key] = objamage[key].reshape(
                -1, objamage[key].shape[-1]
            )
    ...
    if prune_verts:
        ...
        meshing_ret["vert"], meshing_ret["face"], meshing_ret["uv"] = (
            vert,
            face,
            uv,
        )
...
def preview_omg(
    omg_tensor,
    geometry_only=None,
    omg_keys=["position", "occupancy", "objnormal", "color", "metal", "rough"],
    scale_omg_to=None,
    return_rendered=True,
    combine_vomgs=True,
    resolution=512,
    render_kwargs=dict(shadow_catcher=True, samples=32, camera_position=None),
    seg_color_palette=None,
    decoder_kwargs=dict(render_mode="segcolor"),
):
    from xgutils import bpyutil, glbutil

    if scale_omg_to is not None:  # can be (64, 64)
        # use nearest neighbor to resize the omg_tensor
        omg_tensor = skimage.transform.resize(
            omg_tensor,
            scale_omg_to,
            order=2,
        ...
        omg_tensor[..., 3] = (omg_tensor[..., 3] > 0.1).astype(np.float32)
    ...
    img = skimage.transform.resize(
        img, (resolution, resolution * img.shape[1] / img.shape[0]), order=0
    )
    geometry_only = (
        (omg_tensor.shape[-1] == 4) if geometry_only is None else geometry_only
    )
    if return_rendered:
        # print(np.minimum(omg['occupancy'], 1-omg['occupancy']).max())
        # visutil.showImg( np.minimum(omg['occupancy'], 1-omg['occupancy']) )
        obj, meshing_ret = objamage_decoderv2(
            omg, geometry_only=geometry_only, **decoder_kwargs
        )
        ...
        if nface.shape[0] == 0:
            ...
            rdimg = bpyutil.render_scene(
                obj=obj, resolution=(resolution, resolution), **render_kwargs
            )
            if combine_vomgs:
...
def downsample_omg(omg, factor=16, anti_aliasing=False, visualize=False):
    ...
    )  # sqaure strucure is needed to get the corners
    ...
    star_occ = (omg_down[..., 3] >= 0.5) | edge_occ_down

    sov = omg_down[..., 3] >= 0.5  # for visualizaton
    ...
    if visualize == True:
        ...
        plt.show()
        vomg0, rdimg, _ = omgutil.preview_omg(
            omg,
            return_rendered=True,
            geometry_only=False,
            combine_vomgs=True,
            decoder_kwargs=dict(
                render_mode="segcolor", preset_blend=PRESET_BLEND
            ),
        )
        vomg1, rdimg, _ = omgutil.preview_omg(
            omg_down,
            return_rendered=True,
            geometry_only=False,
            combine_vomgs=True,
        ...
        vomg2, rdimg, _ = omgutil.preview_omg(
            omg_down_star,
            return_rendered=True,
            geometry_only=False,
            combine_vomgs=True,
...
def objamage_decoderv2(
    omage,
    save_path=None,
    boundary_zipping=False,
    boundary_zipping_eps=5e-2,
    preset_blend=PRESET_BLEND,
    geometry_only=False,
    render_mode="",
    export_glb=True,
    ...
    **kwargs,
    ...
    if nface.shape[0] > 0:
        ...
        obj = bpyutil.mesh_from_pydata(
            nvert, nface, meshing_ret["uv"], name=obj_name
        )
        ...
        if not geometry_only:
            ...
            if color is not None:
                ...
                img_node.image = bpyutil.npimg2bpy(
                    color[..., :3],
                    bpy_img=f"{obj_name}_omage_color",
                    use_sRGB=True,
                )
            ...
            if normal is not None and "normal" not in disable_maps:
                img_node = nodes.new(type="ShaderNodeTexImage")
                img_node.image = bpyutil.npimg2bpy(
                    normal[..., :3], bpy_img=f"{obj_name}_omage_normal"
                )
                ...
                links.new(
                    img_node.outputs["Color"], normal_map_node.inputs["Color"]
                )
            ...
            if metal is not None and "metal" not in disable_maps:
                img_node = nodes.new(type="ShaderNodeTexImage")
                img_node.image = bpyutil.npimg2bpy(
                    metal[..., :1], bpy_img=f"{obj_name}_omage_metal"
                )
                ...
                links.new(
                    sep_node.outputs["Red"], bsdf_node.inputs["Metallic"]
                )
            ...
            if rough is not None and "rough" not in disable_maps:
                img_node = nodes.new(type="ShaderNodeTexImage")
                img_node.image = bpyutil.npimg2bpy(
                    rough[..., :1], bpy_img=f"{obj_name}_omage_rough"
                )
                # add a color separation node
                sep_node = nodes.new(type="ShaderNodeSeparateColor")
                # link the image texture node to the color separation node
                ...
                # link the color separation node to the bsdf node
                links.new(
                    sep_node.outputs["Red"], bsdf_node.inputs["Roughness"]
                )

            # set color image as srgb
            bpy.ops.image.save_all_modified()  # save all modified images
            # must put this line after save_all_modified, or else the image will just be black for unknown reason.
            bpy.data.images[
                f"{obj_name}_omage_color"
            ...
            if "posmap" in render_mode:
                ...
                img_node.image = bpyutil.npimg2bpy(
                    posmap[..., :3],
                    bpy_img=f"{obj_name}_omage_position",
                    use_sRGB=False,
                )
                links.new(
                    img_node.outputs["Color"], bsdf_node.inputs["Base Color"]
                ...
                bpy.ops.image.save_all_modified()  # save all modified images
                # must put this line after
                bpy.data.images["Objamage_pos"].colorspace_settings.name = (
                    "sRGB"
                )
            if "segcolor" in render_mode:
                ...
                if "dist_transform" in render_mode:
                    ...
                    occ = omage["occupancy"][..., 0] > 0.5
                    # important to use order=0 to preserve the binary nature of the occupancy map
                    occ = skimage.transform.resize(
                        occ,
                        (512,) * 2,
                        order=0,
                        preserve_range=True,
                    ...
                    segcolor = skimage.transform.resize(
                        segcolor,
                        (512,) * 2,
                        order=1,
                        preserve_range=True,
                    ...
                    df = scipy.ndimage.distance_transform_edt(
                        occ.astype(float)
                    )
                    df_intensity = (
                        np.sin(df / df.shape[0] / 2 / np.pi * 800) ** 2 / 2
                    ) + 0.5
                ...
                img_node.image = bpyutil.npimg2bpy(
                    segcolor,
                    bpy_img=f"{obj_name}_omage_segcolor",
                    use_sRGB=True,
                )
                links.new(
                    img_node.outputs["Color"], bsdf_node.inputs["Base Color"]
        ...
        if save_path is not None:
            if export_glb:
                ...
                bpy.ops.export_scene.gltf(
                    filepath=save_path.replace(".blend", ".glb"),
                    export_format="GLB",
                    use_selection=True,
                )
            else:
...
def remove_small_patches(obj, min_area=1e-5, max_n_patch=128):
    ...
    # assuming component ids have no gaps (not like 0,1,3,4)
    f_component_id = igl.facet_components(of)
    ...
    remove_component_mask = (component_area_2d < min_area) | (
        component_area_3d < min_area
    )
    ...
    mesh = obj.data
    bpy.ops.object.mode_set(mode="EDIT")
    # deselect all verts, edges as well.
    ...
    meta_info = dict(
        n_patch_original=n_patch_original,
        n_patch_removed=n_patch_removed,
        n_patch_final=n_patch_final,
    )
...
def repacking_uv2(
    ...
    bpy.ops.mesh.remove_doubles(
        threshold=1e-6, use_unselected=False, use_sharp_edge_from_normals=True
    )  # already selected all verts

    # remove islands with area less than 0.0001 and only keep the largest K islands
    # remove small islands
    _, _, meta_info = remove_small_patches(
        obj, min_area=min_area, max_n_patch=max_n_patch
    )

    # select all uv
    bpy.ops.object.mode_set(mode="EDIT")
    # It turns out you have to select multiple uv verts if sync is turned on, which may cause problem for uv repacking operations.
    bpy.context.scene.tool_settings.use_uv_select_sync = False
    bpy.ops.mesh.select_all(action="SELECT")
    bpy.ops.uv.select_all(action="SELECT")

    if rescale_area == True:
        # scale all islands according to their area in 3D. For .glb with multiple objects, this is necessary as the UV islands are not scaled properly across objects.
...
def setup_baking(node_tree, resolution=10, bake_target="position"):
    ...
    # object mode
    bpy.ops.object.mode_set(mode="OBJECT")
    # disable denoising to speed up baking
    bpy.context.scene.cycles.use_denoising = False

    # large margin size will make the baking slower. Small margin (e.g. 2) will cause artifacts when downscale the objamage.
    baking_kwargs = dict(
        type="EMIT",
        pass_filter={"EMIT"},
        use_clear=True,
    ...
    if bake_target == "objnormal" or bake_target == "position":
        ...
        if bake_target == "objnormal":
            ...
            links.new(
                geo_node.outputs["Position"], output_node.inputs["Surface"]
            )
    else:
        # removed normal. use objnormal instead.
        mapper = dict(color="Base Color", metal="Metallic", rough="Roughness")
        ...
        if len(bsdf_node.inputs[mapper[bake_target]].links) == 0:
            ...
            value_node.outputs[node_output].default_value = bsdf_node.inputs[
                mapper[bake_target]
            ].default_value
            # if bake_target == 'color':
            #     for i in range(3):
            ...
            links.new(
                source_node.outputs[source_socket.name],
    ...
    if uv_map_node is None:
        ...
        uv_map_node.uv_map = (
            "Repack"  # Assuming'Repack' is the name of the new UV Map
        )
...
def baking_obj(obj, resolution=10, bake_target="position"):
    ...
    for mat in obj.data.materials:
        if mat.use_nodes:
            baking_kwargs = setup_baking(
                mat.node_tree, resolution=resolution, bake_target=bake_target
            )
...
def baking_uv(obj, resolution=10):
    ...
    bpy.ops.object.mode_set(mode="EDIT")
    bpy.ops.mesh.select_all(action="SELECT")
    # It turns out you have to select multiple
    bpy.context.scene.tool_settings.use_uv_select_sync = False
    # activate the default uv map
    obj.data.uv_layers["UVMap"].active_render = True

    # set up baking settings
    bpy.context.scene.render.engine = "CYCLES"
    # baking surface properties does not need high samples
    ...
    objamage = dict(
        objnormal=None, position=None, color=None, metal=None, rough=None
    )
    for bake_target in objamage:
        objamage[bake_target] = baking_obj(
            obj, resolution=resolution, bake_target=bake_target
        )
    # occupancy is objnormal booleaned. if trunormal is 0 vector then it is 0, otherwise 1
    objamage["occupancy"] = (
        np.linalg.norm(objamage["objnormal"][..., :3], axis=-1) > 0
    )[..., None]
    # normalize to [0, 1]
    objamage["position"] = (objamage["position"][..., :3] + 1) / 2
    # remove alpha channel
    objamage["objnormal"] = objamage["objnormal"][..., :3]

    return objamage  # all images are in [0, 1] range


def objamage_encoderv2(
    glb_path,
    resolution=10,
    min_patch_area=1e-4,
    max_n_patch=128,
    repack_margin=0.02,
    repacking_margin_method="FRACTION",
    save_path=None,
    rescale_area=False,
    use_uint16=False,
    subdiv=0,
    ...
    timer.update("loading preset blend")
    bpyutil.clear_collection("workbench")
    # load glb, join into a single mesh and put it to workbench collection
    obj = bpyutil.load_glb(glb_path)
    if subdiv > 0:
    ...
    obj = bpy.context.active_object
    bpyutil.set_active_exclusive(obj)
    meta_info = repacking_uv2(
        obj,
        margin=repack_margin,
        min_area=min_patch_area,
        max_n_patch=max_n_patch,
        margin_method=repacking_margin_method,
        rescale_area=rescale_area,
    )
    ...
    if save_path is not None:
        if save_path.endswith(".ply"):
            ...
            igl.write_triangle_mesh(
                save_path, vert, face, uv, force_ascii=False
            )
