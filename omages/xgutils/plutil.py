    ...
    with torch.no_grad():
        ...
        for ind in data_indices:
            ...
            for key in dataitem:
                ...
                if (
                    type(datakey) is not np.ndarray
                    and type(datakey) is not torch.Tensor
                ):
                    continue
...
def dataset_generator(
    pl_module, dset, data_indices=[0, 1, 2], yield_ind=True, **get_kwargs
):
    is_training = pl_module.training
    with torch.no_grad():
        pl_module.eval()
...
class VisCallback(Callback):
    ...
    def __init__(
        self,
        visual_indices=[0, 1, 2, 3, 4, 5],
        all_indices=False,
        force_visual_indices=False,
        every_n_epoch=3,
        ...
        parallel_vis=False,
        single_vis=False,
        visall_after_training_end=True,
        end_command="",
        **kwargs,
    ):
        super().__init__()
    ...
    def process(
        ...
        if parallel_vis == True:
            visual_indices = get_effective_visual_indices(
                visual_indices, pl_module.global_rank, self.num_gpus
            )
            print(
                "global_rank",
                self.pl_module.global_rank,
                "/",
                self.num_gpus,
                "effecitve indices",
                visual_indices,
        ...
        if self.use_dloader == False:
            ...
            datagen = dloader
        computegen = FlyObj(
            data_processor=self.compute_batch,
            save_dir=compute_dir,
            load_dir=cload_dir,
            on_the_fly=fly_compute,
        )
        visgen = ImageFlyObj(
            data_processor=self.visualize_batch,
            save_dir=visual_dir,
            load_dir=vload_dir,
        )
        ...
        for ind in sysutil.progbar(visual_indices):
            try:
                ...
                if debug == True:
                    ...
                    pdb.post_mortem(
                        exc_traceback
                    )  # Pass the traceback to post_mortem
        ...
        np.savetxt(
            failed_log + f"/rank_{self.pl_module.global_rank}.txt",
            np.array(failed_ind),
        )
        # split the (world_size, N) images
        if visual_summary == True and len(imgs) > 0:
            # all_images = imgs
            summary_imgs = self.get_summary_imgs(pl_module, imgs, zoomfac=1.0)
        ...
        if self.pl_module.global_rank == 0:
            ...
            if hasattr(pl_module, "current_epoch"):
                html_dir = os.path.join(
                    data_dir, f"visual_html_{pl_module.current_epoch}/"
                )
            else:
    ...
    def visualize_batch(computed):
        ...
        Ytg = computed["logits"]
        Ytg = batch["Ytg"]
        vert, face = geoutil.array2mesh(Ytg, thresh=0.5)
    ...
    def on_sanity_check_end(self, trainer, pl_module):
        ...
        if self.single_vis == True and pl_module.global_rank != 0:
            print(
                "single_vis==True, Only run callback on global_rank 0, current rank:",
        ...
        self.process(
            pl_module,
            trainer.datamodule.visual_dataloader(),
            visual_summary=True,
        )
    ...
    def on_train_epoch_end(
        ...
        if (
            trainer.current_epoch % self.every_n_epoch
    ...
    def on_test_start(self, trainer, pl_module, **kwargs):
        self.process_all(
            pl_module, trainer.datamodule.visual_dataloader(), **kwargs
        )
    ...
    def post_training_process(self, trainer, pl_module, data_module, **kwargs):
        if self.visall_after_training_end == True:
            self.process_all(
                pl_module, data_module.visual_dataloader(), **kwargs
            )

    def get_summary_imgs(self, pl_module, imgs, zoomfac=0.5):
        all_images = []
        rows = len(imgs)
        ...
        # import pdb
        # pdb.set_trace()
        summary = visutil.imageGrid(
            all_images, shape=(rows, -1), zoomfac=zoomfac
        )
        return {self.classname: {"caption": self.classname, "image": summary}}

    def log_summary_images(
        self, trainer, pl_module, summary_imgs, x_axis="epoch"
    ):
        # wandb logger
        import wandb
...
class ComputeGraphCallback(Callback):
    ...
    def define_nodes_graph(self):
        raise NotImplementedError(
            "Please define the nodes in the compute graph"
        )
        ...
        **kwargs,
        ...
        for i in range(len(self.nodes)):
            ...
            if self.nodes[i]["type"] == "normal":
                gen = FlyObj(
                    data_processor=self.nodes[i]["fn"],
                    save_dir=save_dir,
                    load_dir=(
                        save_dir if self.nodes[i]["load"] == True else None
                    ),
                    on_the_fly=fly_compute,
                )
...
def get_debug_model(trainer, resume=False, device="cuda"):
    if resume == True:
        ...
        pl_model, train_dloader, val_dloader, test_dloader = trainer.test_mode(
            resume_from=None, device=device
        )
    train_dloader.num_workers = (
        0  # it will be very slow to invoke subprocesses (num_workers>0)
    )
    val_dloader.num_workers = (
        0  # it will be very slow to invoke subprocesses (num_workers>0)
    )
...
def debug_model(
    ...
    pl_model, train_dloader, val_dloader, test_dloader = get_debug_model(
...
def test_batch(pl_model, train_dloader, val_dloader, test_dloader):
    exit_flag = False
    try:
        th_train_batch = ptutil.nps2ths(
            next(iter(train_dloader)), device="cuda"
        )
        ...
        loss = (
            pl_model.training_step(th_train_batch, batch_idx=0).detach().item()
        )
        ...
        #    optimizer.step()
        loss = (
            pl_model.validation_step(th_val_batch, batch_idx=0).detach().item()
        )
