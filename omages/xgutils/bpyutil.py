...
def load_blend(path, verbose=False):
    if path[0] != "/" and path[0] != "~":
        print(
            "Warning: path is not absolute, trying to convert to absolute path."
        )
    ...
    for i in range(10):
        try:
            ...
            if i == 9:
                ...
                raise e
            print(
                f"Failed to save blend file. Retrying... Retry count: {i+1}/10"
            )
...
def load_glb(glb_path, collection="workbench"):
    ...
    for obj in bpy.context.selected_objects:
        ...
        # set a mesh object as active (in order to join)
        if obj.type == "MESH":
...
def cleanup_unused_data():
    ...
    def remove_unused_data_block(data_block_collection):
        for item in [
            block for block in data_block_collection if block.users == 0
        ]:
...
def npimg2bpy(np_img, bpy_img=None, use_sRGB=False):
    ...
    if np_img.shape[2] == 3:
        np_img = np.concatenate(
            [np_img, np.ones((np_img.shape[0], np_img.shape[1], 1))], axis=2
        )
    if type(bpy_img) is str:
        ...
        if (
            bpy_img.size[0] != np_img.shape[0]
            or bpy_img.size[1] != np_img.shape[1]
        ):
...
def render_scene(
    ...
    output_path=None,
    view_transform="AgX",
    view_look=f"AgX - Very High Contrast",
):  # can not use BLENDER_EEVEE in docker
    ...
    bpy.ops.render.render(write_still=True, layer="ViewLayer")
    print(
        f"Render complete. Image saved to: {bpy.context.scene.render.filepath}"
    )
    img = readImg(output_path)
    return img


def make_PBSDF_vertex_color_mat(
    baseMatName="MetalMaterial",
    newMatName=None,
    color_layer_name="vertex_color",
):
...
def render_mesh(
    vert, face, vert_color=None, preset_blend=preset_glb, **kwargs
):
    ...
    make_PBSDF_vertex_color_mat(
        baseMatName="RoughMaterial", newMatName="rough_vertex_color_mat"
    )
...
def get_polygon(obj):
    ...
    faces = [
        [vertex for vertex in polygon.vertices]
...
def get_trimesh(obj, return_uv=False):
    ...
    # this is to make sure uv is also selected
...
def if_uv_overlap(obj):
    ...
    overlap_detected = any(
        face.select for face in bmesh.from_edit_mesh(obj.data).faces
    )
...
def rescale_uv(obj, min_size=0.02):
...
def mesh2vfe(mesh):
    ...
    faces = [
        [vertex for vertex in polygon.vertices] for polygon in mesh.polygons
    ]
...
def weld_and_decimate(obj, fix_nonmanifold=False, decimate_angle=1):
    ...
    if len(obj.data.polygons) >= 3:
        planar_decimation_modifier = obj.modifiers.new(
            name="Planar Decimation", type="DECIMATE"
        )
        planar_decimation_modifier.decimate_type = "DISSOLVE"
        planar_decimation_modifier.angle_limit = (
            (np.pi * 2) / 360 * decimate_angle
        )
        bpy.ops.object.modifier_apply(modifier=planar_decimation_modifier.name)
    else:
        # enter edit mode
...
def export_mesh(obj, outpath=None):
    ...
    ditem = dict(
        verts=vertices, faces=faces, face_normals=face_normals, f_adj=adj_mat
    )
    if outpath is not None:
        np.savez(outpath, **ditem)
    return ditem
