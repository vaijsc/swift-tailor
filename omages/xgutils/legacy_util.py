...
def imgs2video2(imgs_dir, out_path, frameRate=6):
    ...
    command = command.format(
        outpath=out_path,
        imgs=os.path.join(imgs_dir, "%d.png"),
        framerate=frameRate,
    )
    print("Executing command: ", command)
    _, stderr, returncode = runCommand(command)
...
def array2mesh(array, thresh=0.0, dim=3, coords=None):
    import mcubes
...
def makeGrid(
    bb_min=[0, 0, 0], bb_max=[1, 1, 1], shape=[10, 10, 10], flatten=True
):
...
def shapes2sdfs(
    ...
    args = [indices]
    func = lambda index: shape2sdf(
        shapePath, index, gridDim=gridDim, disturb=disturb
    )
    batchFunc = lambda batchOut: [
        H5Var(sdfPath, "SDF").append(np.array(batchOut))
    ]
    ret = np.array(
