...
# math utils
def lemniscate(
    t=None, start=0.5, end=2.5, num=256, a=1, x_scale=1.0, y_scale=1.0
):
    if t is None:
        t = np.linspace(np.pi * start, np.pi * end, num, endpoint=False)
...
def logistic(x, x0=0.5, L=1, k=20):
    """
...
def s_shape_curve_0to1(x, beta=3.0):
    """ """
    xeps = x + EPS
...
def rescale(vertex):
    ...
    vertex = shifted / scope * 40 + np.array([50.0, 50.0])
...
def setDiffND(array1, array2, precision=1):
    ...
    diff = (
        np.setdiff1d(a1_rows, a2_rows)
        .view(array1.dtype)
        .reshape(-1, array2.shape[1])
    )
    return diff


def split_by_category(
    array: np.ndarray, category: Iterable, order="occurrence"
) -> np.ndarray:
...
def array2chunks(array, chunk_size=2):
    ...
    if oshape[0] // chunk_size * chunk_size != oshape[0]:
        print(
            "Warning! some data will be dropped during array2chunks (len mod chunk_size!=0)"
        )
...
def array2batches(array, chunkSize=4):
    return [
        ...
        for i in range((len(array) + chunkSize - 1) // chunkSize)
    ]
...
def parallelMap(
    ...
    out = []
    iterations = (
        enumerate(batches) if quiet == True else progbar(enumerate(batches))
    )
...
def makeArchive(folder, outpath, format="zip"):
    ...
    if os.path.exists(out_file):
        os.remove(out_file)
    shutil.make_archive(
        base_name=outpath, format=format, root_dir=parent, base_dir=folder_name
    )
...
def arraySupersample(array, weight=None, multiplier=10):
    sup = []
    if weight is None:
        weight = np.linspace(0.0, 1.0, multiplier)
...
def gridInterp(
    queries,
    grid,
    gridRange=np.array([[0.0, 1.0], [0.0, 1.0], [0.0, 1.0]]),
    return_func=False,
):
    axes = [
        np.arange(gridRange[i, 0], gridRange[i, 1], 1.0 / grid.shape[i])
        for i in range(gridRange.shape[0])
    ]
...
def points2grid(points, values, grid, method="linear", nan=0.0):
    ...
    grid_values = scipy.interpolate.griddata(
        points, values, tuple(split_grid_coords), method=method
    )
    return np.nan_to_num(
        grid_values, nan=nan
    )  # replace nan (due to extrapolation) with number
...
def points2grid_unittest():
    def func(points):
        ...
        return (
            x * (1 - x) * np.cos(4 * np.pi * x) * np.sin(4 * np.pi * y**2) ** 2
        )

    points = np.random.rand(3, 2) * 2 - 1
    values = func(points)
    grid = nputil.makeGrid(
        bb_min=(-1,) * 2, bb_max=(1,) * 2, shape=(64,) * 2, flatten=False
    )
...
def array2mesh(array, thresh=0.0, dim=3, coords=None, sigma=None):
    import mcubes
...
def makeGrid(
    ...
    for i, si in enumerate(shape):
        if mode == "on":
            ...
            offset = (bb_max[i] - bb_min[i]) / 2.0 / si
            # 2*si*w=bmax-bmin
            # w = (bmax-bmin)/2/si
        ...
        coords.append(coord)
    grid = np.stack(
        np.meshgrid(*coords, sparse=False, indexing=indexing), axis=-1
    )
    if flatten == True:
...
def array2NDCube(array, N=3, feature_dim=0):
    vox_dim = np.ceil(np.power(array.shape[0], 1.0 / N)).astype(int)
    if feature_dim == 0:
...
def padArray(objArray):
    if (type(objArray) is np.ndarray) and (
        not all((type(obj) is np.ndarray) for obj in objArray)
    ):
        raise ValueError(
            "Invalid objArray! Not every array element is np.ndarray"
        )
...
def serializeArray(objArray, return_dict=False):
    ...
    serialData, serialShape = np.concatenate(serialData), np.concatenate(
        serialShape
    )
...
def deserializeArray(
    serialData, dataBias, serialShape, shapeBias, quiet=False
):
    targetArrays = []
    if quiet == False:
    ...
    def append(self, array):
        ...
        for i, key in enumerate(
            ...
            if oshape == 0:
                ...
                f.f[key][oshape:tshape] = (
                    dataTuple[i][1:] + f.f[key][oshape - 1]
                )
...
class H5File:
    ...
    def get_serial_data(self, key, index):
        ...
        data = np.array(
            serial_data[dataBias[index] : dataBias[index + 1]]
        ).reshape(shape)
        return data
        ...
        if "%s_serial_dataBias" % key in fkeys:
            if len(value) == 1:
                serialData, dataBias, serialShape, shapeBias = (
                    np.array(f[key]),
                    np.array(f["%s_serial_dataBias" % key]),
                    np.array(f["%s_serial_shape" % key]),
                    np.array(f["%s_serial_shapeBias" % key]),
                )
                ...
                if isIterable(value[1]):
                    item = np.array(
                        [self.get_serial_data(key, ind) for ind in value[1]]
                    )
                else:
...
def readply(path, scaleFactor=1 / 256.0):
    ...
    try:
        ...
        face = np.array(
            [
                plydata["face"]["vertex_index"][i]
                for i in range(len(plydata["face"]["vertex_index"]))
            ]
        ).astype(int)
        # vert = np.zeros((2,3))
    except:
