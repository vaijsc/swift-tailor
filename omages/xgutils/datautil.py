...
def gen_datalist(
    ...
    if name_list is None:
        ...
        ditem_rel_paths = [
            os.path.relpath(ditem_path, dset_root)
            for ditem_path in ditem_paths
        ]
    else:
        ditem_paths = [
            dset_root + "/" + data_dir + "/" + name for name in name_list
        ]
    ...
    np.savetxt(
        os.path.join(dset_root, f"{list_name}.txt"), ditem_rel_paths, fmt="%s"
    )
    for i, split in enumerate(splits):
        name = os.path.join(
            dset_root, f"{list_name}_{split_names[i]}.txt".format()
        )
...
def generate_split(N, splits=[0.8, 0.1, 0.1], shuffle=True, seed=314):
...
def batch_generator_for_ranks(
    dataset, ind_list=None, batch_size=1, global_rank=0, world_size=1
):
    ...
    ind_list_ext = np.concatenate(
        [ind_list, np.random.choice(ind_list, N_ext - N, replace=True)]
    )
...
def npyfolder2dict(folder_path, keys=None):
    ...
    if not folder_path.endswith("/"):
        folder_path += "/"
    for rkey in glob.glob(
        folder_path + "*.npy"
    ):  # will ignore names starting with .
        key = rkey.split("/")[-1].replace(".npy", "")
        if keys is not None and key not in keys:
            continue
        ditem[key] = np.load(rkey, allow_pickle=True)
        # check if loaded data is pickled
        if isinstance(ditem[key], np.ndarray) and ditem[key].dtype == np.dtype(
            "O"
        ):
...
def df2dset(df, dset_root, description=""):
    ...
    meta_df = pd.DataFrame(
        [dict(description=description, num_items=len(df), columns=df.columns)]
    )
    meta_df.to_json(
        os.path.join(dset_root, "df_meta.json"), orient="records", indent=4
    )
...
class DatasetProcessor:
    ...
    def __init__(
        self, config, force_overwrite=False, filemode=True, debug=False
    ):
        self.__dict__.update(locals())
        self.ditems = dict()
        ...
        try:
            ...
            if self.force_overwrite == False:
                if self.filemode == True:
                    ...
                    if os.path.exists(ditem_path):
                        loaded_ditem = (
                            pd.read_json(ditem_path).iloc[0].to_dict()
                        )
                else:
            ...
            print(error_message)
            output_ditem = dict(
                _success=False, _error_message=error_message, _valid=False
            )
            if self.debug:
                import pdb
    ...
    def process(self):
        ...
        if self.filemode and self.target_root is not None:
            output_df.to_json(
                self.target_root + "/dataframe.json",
                orient="records",
                indent=4,
            )
        return output_df
