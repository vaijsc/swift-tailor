...
from xgutils import optutil, sysutil
...
class Trainer:
    ...
    def __init__(self, opt, root_dir=DEFAULT_ROOT, mode="test", gpus=[0]):
        ...
        if len(trainer_opt["gpus"]) == 1:
            ...
            if self.trainer_opt["disable_auto_lr_scale"] == False:
                self.opt.pl_model_opt["kwargs"]["optim_opt"]["lr"] *= len(
                    trainer_opt["gpus"]
                )
        ...
        print(
            "!!!!!!!!!!!!!!! Max epochs", self.MainTrainerOptions["max_epochs"]
        )
        ...
        if (
            mode == "train"
            and trainer_opt["accelerator"] == "dp"
            and trainer_opt["auto_lr_find"] == True
            and trainer_opt["resume_from"] == "restart"
        ):
    ...
    def find_lr(self):
        ...
        lr_finder = self.trainer.tuner.lr_find(
            self.model, datamodule=self.data_module, early_stop_threshold=10
        )
        # print(lr_finder.results)
        fig = lr_finder.plot(suggest=True)
        fig.show()
    ...
    def train(self):
        ...
        print(
            "Test!!",
            len(self.data_module.train_set),
    ...
    def test(self, resume_from=None):
        ...
        self.trainer.test(
            self.model, self.data_module.test_dataloader(), ckpt_path=None
        )
    ...
    def test_mode(self, resume_from="last", device="cuda"):
        if resume_from is None:
            ...
            self.model = self.model.load_from_checkpoint(
                resume_from, **self.opt.pl_model_opt["kwargs"]
            )
    ...
    def parse_resume(self, ckpt):
        ...
        if ckpt == "" or ckpt == "restart":
            if (
                "nnrecon/experiments/" not in minfo["expr_dir"]
                or minfo["expr_dir"].endswith("experiments")
                or minfo["expr_dir"].endswith("experiments/")
            ):
                raise ValueError(
                    f"Invalid resume_from: {minfo['expr_dir']}"
