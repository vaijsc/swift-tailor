...
def get_activations(
    pointclouds, model, batch_size=100, dims=1808, device=None, verbose=False
):
...
def calculate_activation_statistics(
    ...
    """
    act = get_activations(
        pointclouds, model, batch_size, dims, device, verbose
    )
...
def save_statistics(real_pointclouds, path, model, batch_size, dims, cuda):
    m, s = calculate_activation_statistics(
        real_pointclouds, model, batch_size, dims, cuda
    )
...
def calculate_fpd(
    pointclouds1, pointclouds2=None, batch_size=100, dims=1808, device=None
):
    """Calculates the FPD of two pointclouds"""
    ...
    m1, s1 = calculate_activation_statistics(
        pointclouds1, model, batch_size, dims, device
    )
    if pointclouds2 is not None:
        m2, s2 = calculate_activation_statistics(
            pointclouds2, model, batch_size, dims, device
        )
