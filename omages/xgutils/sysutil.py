...
def flatten_tree_to_level(tree, k, depth=0, index=(), acc_ind=None):
    ...
    return [
        ...
        for item in flatten_tree_to_level(
            subtree, k, depth + 1, index + (i,), acc_ind
        )
    ]
...
class Timer:
    ...
    def update(self, text="", print_time=True):
        ...
        if print_time and not self.quiet:
            ...
            print(
                self.time_stamps[-1] - self.time_stamps[-2],
                self.time_stamps[-1] - self.time_stamps[0],
            )
...
def mkdir(path):
    ...
    path = os.path.expanduser(
        path
    )  # resolve the ~ symbol at the beginning of the path
...
def pmap(func, args, zippedIn=True, zippedOut=False, cores=-1, quiet=False):
    ...
    ctx._force_start_method(
        "spawn"
    )  # force spawn start method to avoid slow performance
...
def plys2h5(plyDir, h5path, indices=None, scaleFactor=1 / 256.0):
    ...
    def func(path):
        return readply(path, scaleFactor=scaleFactor)
...
def unit_test():
    ...
    checker = sysutil.dictUpdate(a, b) == {
        "a": {"b": 45, "c": 4},
        "d": {"b": 3, "c": 4},
        "df": 0,
